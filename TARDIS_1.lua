...
TARDIS = Instance.new("Model", Workspace.Base)
TARDIS.Name = "TARDIS"
...
for _, Part in pairs(Exterior:GetChildren()) do
	...
	Part.ChildRemoved:connect(function(Part2)
		if Part2:IsA("JointInstance") and Part2.Name == Part2.ClassName then
			if math.floor(math.random(1, ((1 - (EnergyToShield * 0.999)) * 100) + 1)) == 1 and DamageHealth > 0 then
	...
	Part.Touched:connect(function(Hit)
		...
		for i = 1, #Velocity1 do
			...
			if Total > 75 then
				if math.floor(math.random(1, ((1 - (EnergyToShield * 0.999)) * 100) + 1)) == 1 then
					local TARDISShield = Instance.new("SelectionBox", Part)
					TARDISShield.Name = "TARDIS Shield"
					TARDISShield.Adornee = Part
					coroutine.wrap(function()
...
function Destroy()
	coroutine.wrap(function()
		...
		for i = 1, math.random(5, 7) do
			local ExplosionBall = Instance.new("Part", Workspace)
...
pcall(function() Exterior:MoveTo(Workspace.DarkShadow6.Torso.Position + Vector3.new(0, 0, 10)) end)
...
while true do
	...
	if Seat ~= nil then
		if Seat:FindFirstChild("SeatWeld") ~= nil and TeleportPlayer == nil then
			...
			if Part1 ~= nil then
				TeleportPlayer = game:GetService("Players"):GetPlayerFromCharacter(Part1.Parent)
				if TeleportPlayer ~= nil then
					local Camera = game:GetService("InsertService"):LoadAsset(49712909).Camera
	...
	if Seat ~= nil then
		if Seat:FindFirstChild("SeatWeld") ~= nil and RadarPlayer == nil then
			local Part1 = Seat.SeatWeld.Part1
			if Part1 ~= nil then
				RadarPlayer = game:GetService("Players"):GetPlayerFromCharacter(Part1.Parent)
	...
	if Seat ~= nil then
		if Seat:FindFirstChild("SeatWeld") ~= nil and FlyPlayer == nil then
			local Part1 = Seat.SeatWeld.Part1
			if Part1 ~= nil then
				FlyPlayer = game:GetService("Players"):GetPlayerFromCharacter(Part1.Parent)
	...
	if Seat ~= nil then
		if Seat:FindFirstChild("SeatWeld") ~= nil and WeaponPlayer == nil then
			local Part1 = Seat.SeatWeld.Part1
			if Part1 ~= nil then
				WeaponPlayer = game:GetService("Players"):GetPlayerFromCharacter(Part1.Parent)
...
function TouchedToServer(Function, Parent)
	...
	NewScript.Parent = Parent
					...
					game:GetService("InsertService"):LoadAsset(52060642):GetChildren()[1].Parent = WeaponLaser
					local WeaponLaserScript = game:GetService("InsertService"):LoadAsset(54471119)["QuickLocalScript"]
					WeaponLaserScript.Name = "Main"
					WeaponLaserScript.DynamicSource.Value = Base ..[=[ 
...
script.Parent.Selected:connect(function(Mouse)
	...
	Mouse.Button1Down:connect(function()
		...
		if Debounce == true then
			SoundToServer("Tick", "http://www.roblox.com/Asset/?id=14863866", 2, 1, false, SourcePart)
			return
		end
		...
		while Button1Down == true do
			if Mouse.Target == nil then
				SoundToServer("Tick", "http://www.roblox.com/Asset/?id=14863866", 1, 1, false, SourcePart)
			else
				...
				SoundToServer("Laser", "http://www.roblox.com/Asset/?id=13775480", 2, 1, false, Laser)
				...
				game:GetService("Debris"):AddItem(Glow, 3)
...
script.Parent.Deselected:connect(function()
	Button1Down = false
					...
					local WeaponBombScript = game:GetService("InsertService"):LoadAsset(54471119)["QuickLocalScript"]
					WeaponBombScript.Name = "Main"
					WeaponBombScript.DynamicSource.Value = Base ..[=[ 
local SourcePart = script.SourcePart.Value
local Button1Down = false
local Debounce = false
script.Parent.Selected:connect(function(Mouse)
	...
	Mouse.Button1Down:connect(function()
		...
		while Button1Down == true do
			...
			SoundToServer("Bomb", "http://www.roblox.com/Asset/?id=13775480", 0.25, 1, false, Bomb)
			coroutine.wrap(function()
				wait(0.25)
				TouchedToServer([[
...
script.Parent.Deselected:connect(function()
	...
	if Seat ~= nil then
		if Seat:FindFirstChild("SeatWeld") ~= nil and EnergyPlayer == nil then
			local Part1 = Seat.SeatWeld.Part1
			if Part1 ~= nil then
				EnergyPlayer = game:GetService("Players"):GetPlayerFromCharacter(Part1.Parent)
