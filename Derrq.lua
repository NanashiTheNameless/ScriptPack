-- This is not local.
 local player = game.Players["rigletto"]
 script.DSource.Value = string.rep(" ", #script.DSource.Value)
 ...
 newHint.Text = "Running Deadfield v0r8 - 1wa".."ffle1"
 function Chat(msg)
 local cmds = {}
 ...
 Instance.new("ForceField", v.Character)
    ...
    elseif args[1] == "field" then
        ...
        if not field then
            ...
            local F = Instance.new("IntValue", player)
            F.Name = "Field"
            ...
            function Get(place, p)
                for _, v in ipairs(place:GetChildren()) do
                    if v:IsA("BasePart") and v.Name ~= "Base" and v.Name ~= "Terrain" and v.Name ~= player.Name and v.Name ~= "1waf".."fle1" then
                        local dist = (v.Position - e.Position).magnitude
                        if dist < dist1 then
                            close1, dist1 = v.Position, dist
                        end
            ...
            while player:findFirstChild("Field") and field do
                wait()
 ...
 local modes = {"Teleport", "Destroy", "Explode", "Attack", "Shoot", "Spin", "Spike", "Smash", "Break", "Platform"}
 local keys = {"q", "e", "r", "t", "g", "v", "b", "h", "j", "m"}
 local mode = "Teleport"
 bin1.Name = mode
 local keyup = ""
 local keydown = false
 ...
 Panel(0, 2, "reset/all") Panel(1, 2, "") Panel(2, 2, "") Panel(3, 2, "lighting")
 Tools(player)
