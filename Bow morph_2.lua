...
Meshes = {
	Hat = {Asset..(1043438), Asset..(1043443)},
	Torso = {Asset..(52168619), Asset..(33308631)},
	Rarm = {Asset..(52168619), Asset..(33308774)},
	Larm = {Asset..(52168619), Asset..(33308496)},
	Rleg = {Asset..(52168619), Asset..(33308827)},
	Lleg = {Asset..(52168619), Asset..(33308564)},
}
...
HatHandle = Part(Hat, false, false, 0, 0, "White", 1.5, 1.8, 1.4)
HatHandle.Name = "Handle"
Hat.AttachmentPos = Vector3.new(0, -0.25, 0.05)
Add.Mesh(HatHandle, Meshes.Hat[2], Meshes.Hat[1], Vector3.new(1.2,1,1.1))
...
FTorso = Part(Mo, false, false, 0, 0, torso.BrickColor.Name, 2, 2, 1)
FWeld  = Weld(torso, FTorso, 0, 0, 0, 0, 0, 0)

for _,v in pairs({{Rarm, Meshes.Rarm}, {Larm, Meshes.Larm}, {Rleg, Meshes.Rleg}, {Lleg, Meshes.Lleg}, {FTorso, Meshes.Torso}}) do
...
function ShootArrow(pos, power, targ)
	...
	for i = Go, Dist, Go do
		...
		if hit or hit2 then
			...
			if h == nil and realhit.Parent.Parent ~= game and realhit.Parent.Parent ~= nil and realhit.Parent.Parent ~= workspace then
				h = getHumanoid(realhit.Parent.Parent)
			end
			local sound = Sounds.Stick
			if h and hit.Parent.className ~= "Hat" then
				local d = MRA(12+(power/8), 20+(power/5.5))
	...
	mouse.Button1Down:connect(function()
		if Reloading == false and Slashing == false and Shooting == false then
			if ArrowOn == false then
				...
				if yesh then
					local mm = Special >= 50
					if ah and mm or Keys.g and mm then
						SpecialAtk(mouse)
					else
						Shoot(mouse)
					end
