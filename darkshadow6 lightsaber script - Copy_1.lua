...
function onPlasmaTouched(Source, Hit)
	...
	if Humanoid ~= nil and Humanoid.Health > 0 then
		...
		Plas = Player.Character[ModelName]:FindFirstChild("Plasma")
		...
		Dust.CFrame = Plas.CFrame * CFrame.new(math.random(-1000, 1000) / 1000, math.random(-2000, 2000) / 1000, math.random(-1000, 1000) / 1000)
...
function CreateParts(Parent, Format)
	...
	local Plasma = Instance.new("Part", Parts)
	Plasma.Material = "SmoothPlastic"
	Plasma.Name = "Plasma"
	Plasma.BrickColor = BrickColor.new("Institutional white")
	Plasma.BottomSurface = 0
	Plasma.TopSurface = 0
	Plasma.FormFactor = "Custom"
	Plasma.Size = Vector3.new(0.2, 3, 0.2)
	Instance.new("CylinderMesh", Plasma).Scale = Vector3.new(1, 0, 1)
	local Weld = Instance.new("Weld", Plasma)
	Weld.Part0 = Weld.Parent
	Weld.Part1 = Handle
	Weld.C0 = CFrame.new(0, -2.25, 0)
	Weld.C1 = CFrame.new(0, -2.25, 0)
	
	Plight = Instance.new("PointLight",Plasma)
	Plight.Color = Color3.new(255, 255, 0)
	Plight.Range = 10
	Plight.Enabled = false

	Plasma.Touched:connect(function(Hit) onPlasmaTouched(Part, Hit) end)
...
function onKeyDown(Key, Mouse)
	...
	if Button1Down == false and CanUse == true and CheckPlayer() == true then
		...
		elseif Key == "e" then
			CanUse = false
			PL = Player.Character[ModelName]:FindFirstChild("Plight")
			Plight.Enabled = true
