--Tinfold
--If you log this, don't leak it
--Apparently an older version of this script was already leaked. If you get this one, please don't leak it. Leak it when it's done, for god's sake.
--Riven
...
moveDescriptions={
    ...
    ['Origin Beam']={move='Origin Beam',key='LMB, LMB, E',desc=[[Combo move: charges a beam that is held for a short time and deals rapid damage.]]},
    ['Nightmare Overture']={move='Nightmare Overture',key='Z',desc=[[Ultimate move: Four pillars rise, charges a diamond levitates the user. Creates four diamonds than can be shot, dealing massive damage. Final diamond explodes, dealing massive damage and the Curse debuff to anyone inside the blast radius. ]]}
}
...
--local testtorso=cp(tube,'Bright red',Vector3.new(2.8,0.4,1.1))
...
executeMove=function(name)
    if canExecuteMoves==true then
        if ch and hum and hum.Health>0 then
            ...
            if name=='Nightmare Overture' then
                if Vector3.new(0,tor.Velocity.Y,0).magnitude>2 or Vector3.new(tor.Velocity.X,0,tor.Velocity.Z).magnitude>2 then
                    can=false
                end
            end
            if can==true then
...
addMove('Dark Lock','mb1mb1t',1,function()
    tor.CFrame = CFrame.new(tor.Position, Vector3.new(mouse.Hit.p.X, tor.Position.Y, mouse.Hit.p.Z))
    local lv=tor.CFrame
    tweenTable={}
    animPlaying=true
                   ...
                   if can==true then
            	     ...
            	     game.Debris:AddItem(db,0.2)
            	      ...
            	      if humin then
                	      ...
                	      testvel.Velocity=Vector3.new()
                	      game.Debris:addItem(testvel,1)
                	      local fxb=cp(tube,'Really black',Vector3.new(10,10,10),true)
                	      fxb.CFrame=spike.CFrame*CFrame.new(0,5,0)
                	      fxb.Transparency=0.5
                	      fxb.Material='Neon'
                	      local blm=blo(fxb)
                	      blm.Scale=Vector3.new(0,0,0)
                	      addFx(fxb,0.1,nil,function() blm.Scale=blm.Scale+Vector3.new(0.1,0.1,0.1) end)
                	      Delay(3.4,function()
            	            dealDamage(v.Parent,10,5,2)
            	            addFx(fxb,0.07,'destroy',function() blm.Scale=blm.Scale+Vector3.new(0.07,0.07,0.07) fxb.Transparency=fxb.Transparency-0.035 end)
            	        end)
                	      humin.Sit=true
...
addMove('Diamond Storm','v',10,function()
    ...
    repeat wait(0.1)
        ...
        if d then
            ...
            sw.CFrame=CFrame.new(righthandle.CFrame.p, Vector3.new(m.Hit.p.X,m.Hit.p.Y,m.Hit.p.Z))*ang(mr(math.random(-math.random(3.4),math.random(3.4))),mr(math.random(-math.random(3.4),math.random(3.4))),mr(math.random(-math.random(3.4),math.random(3.4))))--*ang(mr(90),0,0)
            ...
            local nct=sw.Touched:connect(function(part)
                if part.Parent and part.Parent:findFirstChild('Humanoid') and not part:IsDescendantOf(ch) and not dmgdealt then
                    dealDamage(part.Parent,6.5,5,2,2)
                    dmgdealt=true
                end
...
addMove('Origin Beam','mb1,mb1,e',12,function()
	...
	addFx(beam,0.02,nil,function()
	   ...
	   -- if mn then
            ...
            local bm=blo(brck)
            addFx(brck,0.07,'destroy',function() bm.Scale=bm.Scale+Vector3.new(0.1,0.1,0.1) brck.Transparency=brck.Transparency+0.07 end)
        --end
    end)
	wait(1)
	addCoolDown('Origin Beam')
	addFx(beam,0.07,'destroy',function() beam.Transparency=beam.Transparency+0.07 msh.Scale=msh.Scale-Vector3.new(0.07,0,0.07) end)
	animPlaying=false
	deb=false
	bg=false
	apoc.Volume=1
...
addMove('Nightmare Overture','z',60,function()
    if Vector3.new(0,tor.Velocity.Y,0).magnitude<2 and Vector3.new(tor.Velocity.X,0,tor.Velocity.Z).magnitude<2 then
        hum.WalkSpeed=0
        animPlaying=true
        local ev
        ...
        spawn(function() --crown
            ...
            local crn=cp(tube,'Really black',Vector3.new(1,1,1),true)
                ...
                --angle=angle+1
                --circ.CFrame=circ.CFrame*ang(0,mr(1),0)
                addFx(crn,0.07,'destroy',function() crn.Transparency=crn.Transparency+0.07 ms.Scale=ms.Scale+Vector3.new(0.8,0.05,0.8) end)
                local bgmc=bigmagic:clone()
                bgmc.Parent=ch.Head
                bgmc.Volume=0.3
                bgmc.Pitch=0.8
        ...
        for _,v in pairs(stakes) do
           ...
           local nsnd=getSound(231917773)
            ...
            snd.Pitch=1.3
            snd:Play()
           addFx(v,0.05,'destroy',function() v.mesh.Scale=v.mesh.Scale-Vector3.new(0.05,0.05,0.05) v.Transparency=v.Transparency+0.05 end)
        end
        local dm=1
        local lastd=false
        ev=m.Button1Down:connect(function()
            if dm~=5 then
                ...
                addFx(pulse,0.1,'destroy',function() pulse.mesh.Scale=pulse.mesh.Scale+Vector3.new(0.4,0.4,0.4) pulse.Transparency=pulse.Transparency+0.1  end)
                ...
                addMover(di,0.8,function()
                    ...
                    addFx(fxp,0.07,'destroy',function() bmesh.Scale=bmesh.Scale+Vector3.new(0.2,0.2,0) fxp.Transparency=fxp.Transparency+0.07 end)
                end)
                di.Touched:connect(function(part)
                    if part:IsDescendantOf(ch)==false then
                    	   ...
                    	   if v and v.Parent and v.className=='Part' and v.Parent:findFirstChild('Humanoid') then
                               ...
                               if can==true then
                        	     ...
                        	     game.Debris:AddItem(db,0.5)
                        	      ...
                        	      if humin then
                            	      local php=humin.Health
                            	      dealDamage(humin.Parent,40,3,2,0.05)
                            	      humin.Sit=true
                            	      humin.PlatformStand=true
                            	      if humin.Health>=php and humin.MaxHealth~=math.huge then
                            	          print('equal')
                       ...
                       -- bfx.Mesh.Scale=Vector3.new(0,0,0)
                        bfx:clearAllChildren()
                        local sp2=sphere(bfx)
                        sp2.Scale=Vector3.new(0,0,0)
                        addFx(bfx,0.007,'destroy',function() sp2.Scale=sp2.Scale+Vector3.new(0.1,0.1,0.1) bfx.Transparency=bfx.Transparency+0.007 end)
                        addFx(blast,0.007,'destroy',function() sp.Scale=sp.Scale+Vector3.new(0.1,0.1,0.1) blast.Transparency=blast.Transparency+0.007 end)
            ...
            repeat wait() 
                ...
                pm.Parent=pulse
                addFx(pulse,0.1,'destroy',function() pm.Scale=pm.Scale+Vector3.new(0.4,0.4,0.4) pulse.Transparency=pulse.Transparency+0.1  end)
            until explodcharg.IsPlaying==false
                circ:Destroy()
                addFx(tdia,0.07,'destroy',function() tdia.Transparency=tdia.Transparency+0.07 end)
                hum.WalkSpeed=regws
                deb=false
                animPlaying=false
                bv:Destroy()
                    ...
                    if v and v.Parent and v.className=='Part' and v.Parent:findFirstChild('Humanoid') then
                               ...
                               if can==true then
                        	     ...
                        	     game.Debris:AddItem(db,0.5)
                        	      ...
                        	      if humin then
                            	      local php=humin.Health
                            	      dealDamage(humin.Parent,.3*humin.MaxHealth,3,2,0.05)
                            	      humin.Sit=true
                            	      humin.PlatformStand=true
                            	      if humin.Health>=php and humin.MaxHealth~=math.huge then
                            	          print('equal')
                       ...
                       -- bfx.Mesh.Scale=Vector3.new(0,0,0)
                        ...
                        addFx(bfx,0.004,'destroy',function() sp2.Scale=sp2.Scale+Vector3.new(0.09,0.09,0.09) bfx.Transparency=bfx.Transparency+0.004 end)
                        addFx(blast,0.004,'destroy',function() sp.Scale=sp.Scale+Vector3.new(0.09,0.09,0.09) blast.Transparency=blast.Transparency+0.004 end)
...
--links
--[[
https://gyazo.com/2bb44f565b143e8025e5cdfc555a641a
https://gyazo.com/20a0fd027484f894623cd8d387196c09
]]
...
--hl/http://httpget-inumeration.c9.io/riven.lua