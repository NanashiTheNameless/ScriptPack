...
for i=1,3 do
Lightning(hit.Position,hit.Position+vt(math.random(-10,10),math.random(-10,10),math.random(-10,10)),5,1,"White",0.3,0.1)
end
...
function GetCube()
if MMouse.Target.Name=="Cube Vertex2" and MMouse.Target.Parent:findFirstChild("Cube Vertex1")~=nil then
attack=true
throwcube=false
print("GOT CUBE")
...
MagicBlock(BrickColor.new("White"),Model["Cube Vertex2"].CFrame,0.5,0.5,0.5,0.4,0.4,0.4)
wait()
end
end),c[i])
end
...
CubeMod=c3[i]
for i=1,2 do
MagicRing(BrickColor.new("White"),CubeMod["Cube Vertex2"].CFrame,1,1,1,math.random(-50,50),math.random(-50,50),math.random(-50,50),0.8,0.8,0.8)
MagicCircle(BrickColor.new("Blue"),CubeMod["Cube Vertex2"].CFrame,1,1,1,2,2,2,0.05)
end
...
local mag=targ.magnitude
if mag<=10 and c2[i].Name~=Player.Name then 
attackdebounce=false
Damagefunc1(head,20,40,0)
end
end
end
end
...
local cube=part(3,Cube,0,0.3,BrickColor.new("Bright blue"),"Cube Vertex2",vt(1,1,1))
cube.CFrame=Torso.CFrame
cubemsh=mesh("BlockMesh",cube,"","",vt(0,0,0),vt(0.7,0.7,0.7))
local actualcube=part(3,Cube,0,0.2,BrickColor.new("White"),"Cube",vt(1,1,1))
...
--                h:TakeDamage(Damage)
                ...
                showDamage(hit.Parent,1,.5,BrickColor.new("Bright blue"))
                end
