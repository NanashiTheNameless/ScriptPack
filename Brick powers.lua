...
-- Sphere bending declarations
local raisingSphere = false
...
local frames = {}
local parts = {}
local Taunts = {"Eat my bl0x", "BLOCK STORM", "We all love blox", "You have BL()X", "Blocks", "SUPER BLOX", "BLOCKS UNITE", "GOOD LUCK Mr.Block", "Coolest block around"}
local raisechats = {"RISE MY BLOCKS", "Get Ready to RISE", "FLY", "Go blocks","flying blocks", "C00L Bl00ks","Blast off blocks","Good Luck on your trip","Fly away blocks"}
...
if part.Name == "Sphere" and not disabled[part] then
...
-- Raise Sphere
if key == "e" and state and not raisingSphere then
...
-- Create Sphere part
local part = createSphere(workspace)
part.Anchored = true
...
elseif key == "c" and state and not raisingSphere then
game:GetService("Chat"):Chat(char.Head, raisechats[math.random(1,#raisechats)], Enum.ChatColor.Red)
raisingSphere = true
while keysDown[key] do
local torsoY = torso.Position.y + 3
...
for _, part in pairs(parts) do
if part.Name == "Sphere" and part ~= lastTower then
part:ClearAllChildren()
part.Anchored = false
...
game:GetService("Chat"):Chat(char.Head, "Sphere, BREAK!", Enum.ChatColor.Red)
for part in pairs(frames) do
if part.Size == Vector3.new(6, 6, 6) then
for x = -1, 2, 2 do
for y = -1, 2, 2 do
for z = -1, 1, 2 do
local oPart = createSphere(workspace)
oPart.Size = Vector3.new(9, 2, 9)
...
for part in pairs(frames) do
if part.Size == Vector3.new(30, 1, 30) then
for x = -3, 2, 2 do
for y = -3, 2, 2 do
for z = -3, 1, 2 do
local oPart = createSphere(workspace)
...
-- Create Sphere
function createSphere(path)
local part = createPart("Sphere", path)
