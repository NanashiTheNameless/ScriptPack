...
function att()
con = tip.Touched:connect(function(hit) local hum = damage(hit.Parent,100,.05,'Bright red')
...
spikes = Instance.new('Model',m)
    ...
    while shooting do
        ...
        p.Touched:connect(function(hit) local h = damage(hit.Parent,100,.1,'Deep blue') if h then fade(p,0,1,.1) p:Destroy() end end)
...
function tablets() local r = 10
    ...
    for i = 36,360,360/30 do
        ...
        p.Name = 'MugTable'
        p.Touched:connect(function(hit) damage(hit.Parent,5000,1,'New Yeller') end)
