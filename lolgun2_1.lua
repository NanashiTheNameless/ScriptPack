...
St.Goo=0
St.Tornado = nil
pcall(function() to.bod:Remove() end)
St.Body = qi{"BodyPosition",to,Name="bod",maxForce=v3(0,0,0)}
u = 0.1
...
so(asset.."3087031",m2,1,0.8)
for i=1,35 do 
if i%5==0 then so(ds.Explode1,bb,1,rn(80,130)/100) end
...
for i=1,5 do wep[i]={} aamz[i] = {} aamz[i].a = 0 aamz[i].ma = 0 aamz[i].db=tick() aamz[i].coo=0 end 
aamz[1].ma = 48
aamz[2].ma = 40
amz = _G.as or aamz
_G.as = amz
...
amz[cwep].coo=(waitz/1) --lastdb = waitz/1
end
...
exm = function(sti,par,coll,cfaa,siz)
local sc = siz or 1
local par = par or sti
local cfa = cfaa or sti.CFrame
for i=1,3 do 
GlowMesh(false,"Sphere",par,v3(rn(10,20)*sc,rn(10,20)*sc,rn(10,20)*sc),coll or 'New Yeller',cfa*cn(rn(-3,3)*sc,rn(-3,3)*sc,rn(-3,3)*sc)*ca(rd(rn(-360,360)),rd(rn(-360,360)),rd(rn(-360,360))),0.1,1,0.1)
end
end
...
GetTarget = function(bb) 
local Dist = 20
local found = {nil,Dist}
local ta
 for i,v in pairs(game:service'Players':GetPlayers()) do
...
ta=v.Character.Head 
local bhit,bpos=ray(bb.Position,ta.Position - (cn(bb.Position,ta.Position)*cn(0,0,-1)).p,Props2)
local fro=cn(bb.Position,bb.CFrame*cn(0,1,0).p):toObjectSpace(ta.CFrame) if not fro or fro.z>10 or math.abs(fro.x)>15 then ta=false end
if (v.Character.Head.Position-bb.Position).magnitude<found[2] and ta and (bpos-ta.Position).magnitude<8 then
 found={v.Character.Head,(bb.Position-v.Character.Head.Position).magnitude}
end
 end 
 end 
 return found[1]
...
rpgs = {} rpgs["rpg"]=1 rpgs["rpg2"]=1 rpgs["rpg3"]=1 rpgs["rpg4"]=1 rpgs["rpg5"]=1 rpgs["mortar"]=1 
exwep = {["goo"]={0.3},["rpg3"]={0.75,nil,5},["rpg4"]={0.3,5,12},["rpg2"]={0,25,14},["rpg5"]={0.3,nil,60},["rpg"]={0.18,25,12},["mortar"]={0.1,15,22},["molotov"]={0.7,nil,18},["grenade3"]={0.7,nil,23},["grenade"]={0.65,nil,16},["grenade2"]={0.7},["tele"]={0.55},["tornade"]={0.7}}
bulletid = -5000
bullets = {}
...
if tmm then bbm=tmm:Clone() bbm.Parent=bb elseif bty~="bullet" then bbm= qi{"SpecialMesh",bb,MeshType="Sphere",Scale=v3(0.6,1.2,0.6)*2*sc} end 
if bty == "bullet" then bb.Name = "bullet" ..Player.Name bbm=qi{"CylinderMesh",bb,Scale=v3(0.2,2,0.2)*2*sc} 
elseif exwep[bty] and rpgs[bty] then if exwep[bty][2] then qi{"Smoke",bb,RiseVelocity=-exwep[bty][2],Size=6,Opacity=0.8,Color=bc("Dark stone grey").Color} end
elseif exwep[bty] and not rpgs[bty] then 
if bty=="goo" then bb.BrickColor=bc("Lime green") bb.Transparency=0.2 end
end 
--St.Accuracy = 0.1
local shak = ca(rd(rn(-St.Accuracy,St.Accuracy)/42)*rang,0,rd(rn(-St.Accuracy,St.Accuracy)/42)*rang)
bb.CFrame=spa*shak*cn(0,2,0)*ca(rd(bty=="rpg5" and 180 or 0),0,0)
local nb = {}
nb.spa = spa nb.dmg = dmg nb.spd = spd nb.ran = ran nb.bty = bty nb.sc = sc nb.bb = bb nb.rang = rang
nb.bhit = nil nb.bpos = v3(0,-500,0) nb.reach=false nb.loop = 0 nb.bbm = bbm nb.invert=bty=="rpg5" and -1 or 1
...
wep[j][np].MaxClip = 5
wep[j][np].MaxAmmo = 8
wep[j][np].func = function(i,wn) so(asset.."16211041",to,1,1)  St.CamShakes=wep[i][wn].Stats[7][1] St.Shake=wep[i][wn].Stats[7][2] FireBullet(cn(handles[1].CFrame*cn(0,0.5,0).p,mous.hit.p)*ca(rd(-90),0,0),12,5,80,"goo",1.4)  amm(1)  gdb(1) end
wep[j][np].Stats={la0[1],la1[1]*ca(rd(-90),rd(48),0)*cn(-0.4,0,0.6),ra0[1],ra1[1]*ca(rd(-90),rd(-25),0)*cn(0.3,0,0.6),{1,1},120,{4,150},1.1}
wep[j][np].Swap = function(self,i) 
local ww= pa(wepp,wepn,"Block",0,0,0,false,false,0,0.125,"") handles[1] = ww 
qi{"Weld",ww,Part0=ra,Part1=ww,C0=cn(-0.1,-1.5,-0.2)*ca(rd(180),0,rd(-22))}
qi{"SpecialMesh",ww,MeshId=asset .. "18265627",TextureId=asset .. "76698655",Scale=v3(1.4,1,1.4)}
end
wep[j][np].Reload = wep[1].Pistol.Reload
wep[j][np].Reclip = wep[1].Pistol.Reclip
np = "Uzi"
...
wep[j][np].MaxClip = 8
wep[j][np].MaxAmmo = 3
wep[j][np].func = function(i,wn) so(asset.."16211041",to,1,0.81) St.CamShakes=wep[i][wn].Stats[7][1] St.Shake=wep[i][wn].Stats[7][2] FireBullet(cn(he.CFrame*cn(0,0,-4).p,mous.hit.p)*ca(rd(-90),0,0),32,2.5,100,"rpg3",1) FirePower.Transparency=0.2 amm(1) gdb(1.2) end
wep[j][np].Stats={la0[1],la1[1]*ca(rd(-90),rd(50),0)*cn(-0.7,0,0.8),ra0[1],ra1[1]*ca(rd(-90),rd(-10),0)*cn(0.3,0,0.6),{1,1},120,{20,250},0.7}
wep[j][np].Swap = function(self,i) 
local ww= pa(wepp,wepn,"Block",0,0,0,false,false,0,0.125,"Black") handles[1] = ww 
qi{"Weld",ww,Part0=ra,Part1=ww,C0=cn(0,-2.1,-1)*ca(rd(-90),rd(0),0)*ca(rd(0),0,0)}  qi{"SpecialMesh",ww,MeshId=asset.."34249026", TextureId=asset.."34249993",Scale=v3(1.6,1.6,1.2)}
...
wep[j][np].MaxClip = 5
wep[j][np].MaxAmmo = 6
wep[j][np].func = function(i,wn) St.CamShakes=wep[i][wn].Stats[7][1] St.Shake=wep[i][wn].Stats[7][2] so(asset .. "48579482",to,1,1) 
local bhit,bpos
local hits = {}
local dir2 = cn(la.CFrame*cn(-1,-3,-1).p,mous.hit.p)
local dir = dir2
local stop=0
for i=1,15 do
...
for x=-1,1,1 do 
for y=-1,1,1 do
bhit2,bpos2=ray(dir.p,dir.p - (dir *cn(0,0,1)).p,Props2)
if x==0 and y==0 then bhit,bpos=bhit2,bpos2 dir=dir*cn(0,0,-(dir.p-bpos).magnitude-1.5) end
if x==0 and y==0 and bhit2 and bhit2.Anchored then stop=1 break end
if bhit2 and bhit2.Parent:findFirstChild("Humanoid") then hits[bhit2.Parent.Name]=true end
end
end
end
for i,v in pairs(hits) do Dmg(i,30) end
local magg = (dir2.p-bpos).magnitude
local lazor =pa(wepp,wepn,"Block",1,1,1,true,true,0.1,0.3,p2.Team~=0 and teamcol[p2.Team] or p2.Color) lazorm = qi{"SpecialMesh",lazor,Scale=v3(3,magg,3)}
lazor.CFrame=cn(dir2.p,bpos)*cn(0,0,-magg/2)*ca(rd(-90),0,0)
de(lazor,3) dlay({},function() lazor.Transparency=lazor.Transparency+(0.7/15) lazorm.Scale=lazorm.Scale-v3(3/15,0,3/15) end,0,15)
amm(1)  gdb(0.7) end
wep[j][np].Stats={la0[1],la1[1]*ca(rd(-90),rd(58),0)*cn(-0.4,0,0.6),ra0[1],ra1[1]*ca(rd(-90),rd(-15),0)*cn(0.3,0,0.1),{1,1},40,{3,180},0.85}
wep[j][np].Swap = function(self,i) 
local ww= pa(wepp,wepn,"Block",0,0,0,false,false,0,0.125,"Dark stone grey") handles[1] = ww 
qi{"Weld",ww,Part0=ra,Part1=ww,C0=cn(0.3,-1.5,-0.6)*ca(rd(180),0,rd(-12))*ca(0,rd(-90),0)} qi{"SpecialMesh",ww,MeshId=asset.."60262041",TextureId=asset.."60262165",Scale=v3(1.5,1.4,1.5)}
FirePower= pa(wepp,wepn,"Block",0,0,0,false,false,1,0.125,"New Yeller")
qi{"Weld",FirePower,Part0=ww,Part1=FirePower,C0=cn(0,2.5,0.4)*ca(rd(0),0,0)} qi{"SpecialMesh",FirePower,MeshId=ds.Crown,Scale=v3(0.2,1,0.2)}
end
wep[j][np].Reload = function(self,i,ma,na) wep[2].Shotgun:Reload(i,ma,na,0.75)  end
wep[j][np].Reclip = wep[1].Pistol.Reclip
j = 3 ------------------
np = "Flamethrower"
wep[j][np] = {}
...
wep[j][np].MaxClip = 1
wep[j][np].MaxAmmo = 1
local nukemesh=qi{"SpecialMesh",nil,Scale=v3(1,1,1)*2,MeshId=asset.."31601976",TextureId=asset.."34020027"}
wep[j][np].func = function(i,wn) 
local scc=0
local perc=0
gmed.Visible=true
Spd[10]=0.1
so(asset.."2101137",to,1,0.4)
gmedt.Text = "!!!!!!!NUCLEAR CHARGE!!!!!!"
while Button and perc<100 and hu.Health>0 do 
perc=perc+1
gmedperc.Text = perc .. "%"
gmedbar.Size = ud((1/100)*perc,-4,1,-4)
...
local scc = perc>=100 and 1 or 0
if scc==0 then
so(ds.Imbue,to,0.9,1)
else
so(asset.."16211041",to,1,0.81) St.CamShakes=wep[i][wn].Stats[7][1] St.Shake=wep[i][wn].Stats[7][2] FireBullet(cn(handles[1].CFrame.p,mous.hit.p)*ca(rd(-90),0,0)*cn(0,2,0),40,3,300,"rpg5",1,1,nukemesh) amm(1) end gdb(30) end
wep[j][np].Stats={la0[1],la1[1]*ca(rd(-90),rd(60),0)*cn(-0.7,0,0.8),ra0[1],ra1[1]*ca(rd(-90),rd(0),0)*cn(0.3,0,0.6),{1,1},40,{20,250},0.7}
wep[j][np].Swap = function(self,i) 
local ww= pa(wepp,wepn,"Block",0,0,0,false,false,0,0.125,"Black") handles[1] = ww 
qi{"Weld",ww,Part0=ra,Part1=ww,C0=cn(0.3,-1.5,-1.1)*ca(rd(180),0,0)*ca(0,rd(-45),0)} qi{"SpecialMesh",ww,MeshId=asset .. "74320147",TextureId=asset .. "74320150",Scale=v3(3,1.4,3)}
end
wep[j][np].Reload = wep[1].Pistol.Reload
wep[j][np].Reclip = wep[1].Pistol.Reclip
np = "Rpg"
...
wep[j][np].MaxClip = 15
wep[j][np].MaxAmmo = 1
wep[j][np].func = function(i,wn) St.CamShakes=wep[i][wn].Stats[7][1] St.Shake=wep[i][wn].Stats[7][2] RocketWoosh=so(asset .. "12222132 ",to,1,1) FireBullet(cn(he.CFrame*cn(0,0,-4).p,mous.hit.p)*ca(rd(-90),0,0),60,0.95,200,"rpg4",1) FirePower.Transparency=0.2  amm(1) gdb(3) end
wep[j][np].Stats={la0[1],la1[1]*ca(rd(-90),rd(60),0)*cn(-0.7,0,0.8),ra0[1],ra1[1]*ca(rd(-90),rd(0),0)*cn(0.3,0,0.6),{1,1},40,{20,250},0.7}
wep[j][np].Swap = function(self,i) 
local ww= pa(wepp,wepn,"Block",0,0,0,false,false,0,0.125,"Black") handles[1] = ww 
qi{"Weld",ww,Part0=ra,Part1=ww,C0=cn(-0.25,-1,-0.5)*ca(rd(-90),rd(-90),0)} qi{"SpecialMesh",ww,MeshId=asset .. "1030777",TextureId=asset .. "1030776",Scale=v3(1,1,1)}
...
wep[j][np].Swap = function(self,i) 
local ww= pa(wepp,wepn,"Block",0,0,0,false,false,0,0.125,"Black") handles[1] = ww 
qi{"Weld",ww,Part0=ra,Part1=ww,C0=cn(0,-1.25,0)*ca(rd(180),0,0)} qi{"SpecialMesh",ww,MeshId=asset.."47679879", TextureId=asset.."47707898",Scale=v3(1.3,1.3,1.6)}
end
wep[j][np].Reload = function(v1,v2,v3,v4) wep[1].Pistol.Reload(v1,v2,v3,v4,handles[1].Mesh,1) end
wep[j][np].Reclip = wep[1].Pistol.Reclip
wep[j][np].func = function(i,wn) 
...
q(function() c12(rw,0,cn(1.5,0.5,0.3),0.1,1,u*0.65)  end) -- throw
c12(rw,1,ra1[1]*ca(rd(-180),rd(0),rd(0)),0.1,1,u*0.8)  
amm(1) FireBullet(cn(he.CFrame*cn(0,0,-5).p,mous.hit.p)*ca(rd(-90),0,0),10,5,100,"tornade",1,nil,handles[1].Mesh) so(ds.Slash,to,1,1)
q(function() c12(rw,0,wep[i][wn].Stats[3],0.1,1,u)  end) -- back
c12(rw,1,wep[i][wn].Stats[4],0.1,1,u)  
St.ArmAiming=true gdb(8)
end
np = "Grenade"
wep[j][np] = {}
wep[j][np].MaxClip = 20
...
q(function() c12(rw,0,cn(1.5,0.5,0.3),0.1,1,u*0.65)  end) -- throw
c12(rw,1,ra1[1]*ca(rd(-180),rd(0),rd(0)),0.1,1,u*0.8)  
amm(1) FireBullet(cn(he.CFrame*cn(0,0,-5).p,mous.hit.p)*ca(rd(-90),0,0),35,3.3,100,"molotov",1,nil,handles[1].Mesh) so(ds.Slash,to,1,1)
q(function() c12(rw,0,wep[i][wn].Stats[3],0.1,1,u)  end) -- back
...
nb.loop=nb.loop+1
local spd2 = nb.spd
nb.bhit,nb.bpos=ray(bb.Position,bb.Position - (bb.CFrame *cn(0,-1*nv,0)).p,Props2)
...
local ta 
if nb.bty=="rpg4" and u%2==0 then ta=GetTarget(bb) end
--c/while wait(0.35) do print((workspace.StereoBan.Torso.CFrame:toObjectSpace(workspace.DoogleFox.Torso.CFrame)).x) end
--Forward
if nb.bty=="rpg4" and type(ta)=="userdata" then 
bb.CFrame=cn(bb.Position,ta.Position)*ca(rd(-90),0,0)*cn(0,spd2,0)
else
bb.CFrame=bb.CFrame*cn(0,spd2*nv,0)*ca(-rd(exwep[nb.bty] and exwep[nb.bty][1] or 0.07),0,0)
end
...
if exwep[nb.bty] and exwep[nb.bty][3] and string.find(nb.bty,"rpg") then BulletHole(nb.bpos,nb.bhit,exwep[nb.bty][3]*nb.sc,ds.Gradient1,Props2) elseif nb.bty=="bullet" then BulletHole(nb.bpos,nb.bhit,1*nb.sc,nil,Props2) end
end 
...
elseif nb.bty and nb.bty == "tornade" then
bb.CFrame=bb.CFrame*cn(0,-2,0) 
bb.Anchored=false
delay(2,function() bb.Anchored=true bb.Name = "Tornado" ..Player.Name
de(bb,15) bb.CFrame=cn(bb.Position)*cn(0,11,0)*ca(rd(180),0,0)*ca(0,rd(rn(-360,360)),0) bb.Transparency=0.1
exm(bb,bb,"White",nil,2) bb.Mesh.MeshId=asset.."36755354" bb.Mesh.TextureId=asset.."39511746" bb.Mesh.Scale=v3(10,11,10)
dlay({},function() dlay({},function() bb.Transparency=bb.Transparency+0.05 end,0,20) end,12)
local bcf = bb.CFrame
local bcfa=ca(0,0,0)
dlay({},function() bcf=bcf*cn(0,0,1.2) bcfa=bcfa*ca(0,rd(-60),0) bb.CFrame=bcf*bcfa end,0.2,12*6)
end)
elseif nb.bty and nb.bty == "rpg5" and bb then
local spa = cn(bb.Position)
local gpos,ghit = FindGround(bb.CFrame) 
if gpos and ghit then BulletHole(gpos.p,ghit,exwep[nb.bty][3]*3,ds.Gradient1,Props2)  end
spa = gpos and cn(gpos.p) or spa
so(asset.."3087031",workspace,1,0.8)
local ri=pa(bb,"","Block",1,1,1,true,true,0.4,0.1,"New Yeller") ri.CFrame=spa*cn(0,25,0)*ca(rd(90),0,0) rim = qi{"SpecialMesh",ri,MeshId=ds.Ring}
dlay({},function() ri.Transparency=ri.Transparency+(0.5/120) rim.Scale=rim.Scale+(v3(1,1,0.5)*2) end,0,160)
dlay({},function() DmgRange(bb,exwep[nb.bty][3],nb.dmg,10) so(ds.Explode1,bb,1,0.85) local rr=30 exm(bb,bb,"New Yeller",cn(bb.Position+v3(rn(-rr,rr),rn(2,rr),rn(-rr,rr)) ),2) end,1,15) de(bb,28)
for i=45,360,45 do
local ww = pa(bb,"Nuke","Block",1,1,1,true,true,1,0,"")  dlay({},function() ww.Smoke.Enabled=false de(ww,9) end,15) 
ww.CFrame=spa*ca(0,rd(i),0)*cn(0,0,15)*ca(rd(10),0,0)
qi{"Smoke",ww,RiseVelocity=18,Size=40,Color=bc("Black").Color,Opacity=0.8}
end
delay(4.5,function()
for i=45,360,45 do
local ww = pa(bb,"","Block",1,1,1,true,true,1,0,"")  dlay({},function() ww.Smoke.Enabled=false de(ww,9) end,15)
ww.CFrame=spa*ca(0,rd(i),0)*cn(0,90,15)*ca(rd(76),0,0)
qi{"Smoke",ww,RiseVelocity=500,Size=50,Color=bc("Black").Color,Opacity=0.8}
end end)
...
elseif nb.bty and nb.bty == "goo" and bb then
local gpos,ghit = FindGround(bb.CFrame) 
if gpos and ghit then
bb.Size=v3(1,1,1) bb.Mesh.Scale=v3(1,1,1) dlay({},function() bb.Mesh.Scale=bb.Mesh.Scale+v3(0.5,0,0.5) end,0.05,10) bb.CFrame=cn(gpos.p) bb.Name="Goo" ..Player.Name
de(bb,20) fade(bb,0.02,10)  
else
bb:Remove()
end
...
GetRegion(to.Position-v3(hra,hra,hra),to.Position+v3(hra,hra,hra),function(v) end,
function(get) 
if get.Name:sub(1,3)=="Goo" then
St.Goo=tick()+4
if u%10==0 then Dmg(Player.Name,4,nil,"Goo",get.Name:sub(4)) end
end
end,true) 
--- TORNADO
local hra = 8
GetRegion(to.Position-v3(hra,hra,hra),to.Position+v3(hra,hra,hra),function(v) end,
function(get) 
if get.Name:sub(1,7)=="Tornado" then
St.Tornado=get
end
end,true) 
end
if St.Tornado and St.Tornado.Transparency<0.8 and dis(to,St.Tornado)<16 then
St.Body.maxForce=v3(1,1,1)*50000
St.Body.position = St.Tornado.CFrame*cn(0,rn(-7,15),rn(4,13)).p
if u%10==0 then Dmg(Player.Name,6,nil,"Tornade",St.Tornado.Name:sub(8)) end
else
St.Tornado = nil
St.Body.maxForce=v3(0,0,0)
end

if St.Goo<tick() then Spd[9]=1 else Spd[9]=0.35 end
