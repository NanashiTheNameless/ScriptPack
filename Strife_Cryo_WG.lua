...
do3 = function()
  ...
  if cooldownmax <= cooldowns[3] and (mana.Value >= 5 or Mode == "Monarch") then
    ...
    for i = #Claw / 2, 1, -1 do
      ...
      while dryice == true and mana.Value > 0 do
          swait()
...
LH.C0 = clerp(LH.C0, cf(-1.05, -0.9, -0.2) * euler(0, -1.57, 0) * euler(0, 0, 0) * euler(-0.06, 0, -0.1), 0.3)
          ...
          if raa % 6 ~= 0 or raa % 3 == 0 then
            hitfloor, posfloor = rayCast(refaa.Position, CFrame.new(refaa.Position, refaa.Position - Vector3.new(0, 1, 0)).lookVector, 20, Character)
...
do4 = function()
  ...
  if cooldownmax <= cooldowns[4] and mana.Value >= 40 and drakes > 0 then
	...
	local deca = Instance.new("NumberValue", Stats.Decrease)
          ...
          if Effects[e] ~= nil then
			...
			do
                      if Thing[2] == "Freeze" then
                        if 0 < Thing[3].Value and Thing[15] ~= nil and Thing[17] ~= nil then
                          Thing[15].Value = 500
                          Thing[17].Value = 500
