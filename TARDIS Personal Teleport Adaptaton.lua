-- DarkShadow6's TARDIS Vortex Manipulator (Personal Teleport Adaptaton)

-- Initial checks.

TeleportPlayer = game:GetService("Players"):FindFirstChild("DarkShadow6")
...
function Teleport(Position)
		...
		for _, PlayerList in pairs(game:GetService("Players"):GetPlayers()) do
			if PlayerList.Character ~= nil and PlayerList ~= TeleportPlayer then
				if PlayerList.Character:FindFirstChild("Torso") ~= nil then
					if (PlayerList.Character.Torso.Position - TeleportPlayer.Character.Torso.Position).magnitude < 5 then
	...
	Sound1.Looped = true
	Sound1:Play()
	local Sound2Part = Instance.new("Part", Workspace)
	Sound2Part.Name = ""
	...
	Sound2Part.CFrame = CFrame.new(TeleportPlayer.Character.Torso.Position)
	for i = 1, #TeleportCharacters do
...
function onEquipped(Mouse)
	...
	TextButton.Parent = TeleportGui.Content
	TextButton.MouseButton1Up:connect(function()
		TeleportGui.Content.X.Text = string.sub(TeleportPlayer.Character.Torso.Position.x, 0, 12)
		TeleportGui.Content.Y.Text = string.sub(TeleportPlayer.Character.Torso.Position.y, 0, 12)
		TeleportGui.Content.Z.Text = string.sub(TeleportPlayer.Character.Torso.Position.z, 0, 12)
	end)
	local TextButton = TextButton:Clone()
	...
	TextButton.MouseButton1Up:connect(function()
		...
		TextButton.MouseButton1Up:connect(function()
			...
			TextButton.MouseButton1Up:connect(function()
				...
				table.insert(TeleportWaypoints, Vector3.new(tonumber(string.sub(TeleportPlayer.Character.Torso.Position.x, 0, 12)), tonumber(string.sub(TeleportPlayer.Character.Torso.Position.y, 0, 12)), tonumber(string.sub(TeleportPlayer.Character.Torso.Position.z, 0, 12))))
				Frame:Remove()
			end)
		end)
		local TextButton = TextButton:Clone()
	...
	coroutine.wrap(function()
		...
		while TeleportGui.Parent ~= nil do
			TeleportGui.Content.CX.Text = TextCX .. string.sub(TeleportPlayer.Character.Torso.Position.x, 0, 12)
			TeleportGui.Content.CY.Text = TextCY .. string.sub(TeleportPlayer.Character.Torso.Position.y, 0, 12)
			TeleportGui.Content.CZ.Text = TextCZ .. string.sub(TeleportPlayer.Character.Torso.Position.z, 0, 12)
			wait()
		end
	end)()
end
