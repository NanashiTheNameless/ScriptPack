...
collist = {
{'LeftLegColor',"Deep blue"},
{'RightLegColor',"Deep blue"},
{'TorsoColor',"Cool yellow"},
{'LeftArmColor',"Cool yellow"},
{'RightArmColor',"Cool yellow"},
}
...
p = T.P(.3,.4,.3,'Cyan',0,false,false,bracs) T.W(p,b1,0,0,.55,0,0,0)
p = T.P(.3,.4,.3,'Cyan',0,false,false,bracs) T.W(p,b1,0,0,-.55,0,0,0)
p = T.P(.3,.4,.3,'Cyan',0,false,false,bracs) T.W(p,b1,.55,0,0,0,math.pi/2,0)
p = T.P(.3,.4,.3,'Cyan',0,false,false,bracs) T.W(p,b1,-.55,0,0,0,math.pi/2,0)
for i,v in pairs(bracs:children()) do if v.Name ~= 'a' then v.Material = 'Ice' end end
end
 
--------MAKING--------------------
h1 = T.P(1.5,1.5,1.5,'Cool yellow',0,false,false)
...
dec.Face = 'Front'
dec.Texture = "http://www.roblox.com/asset/?id=58535182"
 
b2 = T.P(2.1,.5,1.1,'White',0,false,false)
...
if v.Name ~= 'a' then table.insert(fires,T.F(v,.5,.5,'Cyan','Black')) end
end
...
stick = T.P(.3,.3,2.5,'Cyan',.5,false,false,sticks)
stick.Touched:connect(function(hit) damage(hit.Parent,10,true,.05) end)
...
function bowl(mouse)
colorslide(e1,'Color','cur','Earth green',.05)
dec.Parent = e1
light = T.P(1,2,1,'Earth green',.8,false,false)
light.Touched:connect(function(hit) damage(hit.Parent,80,false,1) end)
