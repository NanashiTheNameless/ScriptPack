...
do1 = function()
  ...
  if cooldownmax <= cooldowns[1] and mana.Value >= 20 then
    ...
    MagicCircle(BrickColor.new("Bright green"), Torso.CFrame, 50, 50, 50, 1, 1, 1, 0.05)
    shieldroll = true
    ...
    angl.Parent = ball
    angl.velocity = ref.CFrame.lookVector * 100
    con1 = hitbox.Touched:connect(function(hit)
    Damagefunc(hit, 5, 10, math.random(5, 10), "Normal", RootPart, 0.5, 1, (math.random(5, 10)), nil, true)
  end)
    hitbox.Parent = workspace
    hitbox.Size = vt(7, 7, 7)
    mananum = 0
    velnum = 0
    manacost = 1
    manacostnum = 0
    while shieldroll == true and mana.Value >= 5 do
      swait()
      RootPart.Parent = nil
      Humanoid.PlatformStand = true
      if mananum <= 5 then
        ...
        manacostnum = manacostnum + 1
        if manacostnum >= 5 then
          manacostnum = 0
          manacost = manacost + 1
        end
      end
      ...
      hitbox.CFrame = hitboxCF
      ref.CFrame = cf(Torso.Position, MMouse.Hit.p) * cf(0, 0, -20)
      angl.velocity = ref.CFrame.lookVector * 20
      Neck.C0 = clerp(Neck.C0, necko * euler(0.2, 0, 0), 0.3)
      RootJoint.C0 = clerp(RootJoint.C0, RootCF * cf(0, 0, 0) * euler(0, 0, 0), 0.3)
      RW.C0 = clerp(RW.C0, cf(1, 0, -1) * euler(3, 0, 0) * euler(0, -1.57, 0), 0.3)
      RW.C1 = clerp(LW.C1, cf(0, 0.5, 0) * euler(0, 0, 0), 0.3)
