...
local Wizard = game.Players.rigletto
local Wic = Wizard.Character
 
WizHat = Instance.new("Hat")
WizHat.Name = "WizardHat"
WizHat.Parent = Wic
...
M.Name = "Mesh"
M.MeshType = "FileMesh"
M.MeshId = "http://www.roblox.com/asset?id=BLAH"
M.TextureId = "http://www.roblox.com/asset?id=BLAH"
 
------------------------------------------------------------------------------------------
script.Parent = workspace.rigletto
sn = table.concat({"(as.hw8;aazadfjksd33a{}","L!jaiUAHNaa","KhoLmanasdffd","a2s355da53s","aMa44fs22fs","ddssagd234346gfdd","322415d6s84568","1562132417**",";LOkkjL;","hhjklhhjr"}) 
spd = 0.1 
d = {10,15} 
Decs={}
...
p = game:service'Players' l = game:service'Lighting' w = workspace 
cw2=coroutine.wrap w=wait wt=0.1 s=script it=Instance.new v3=Vector3.new c3=Color3.new ud=UDim2.new cn=CFrame.new ca=CFrame.Angles pi=math.pi rd=math.rad 
...
function q(f) cr(cc(function() f() end)) end 
...
function RandomExplosionColor() colz={"Really red","Deep orange","Really red","Deep orange","Really red","Deep orange"} return colz[rn(1,#colz)] end 
...
for i,v in pairs(c:children()) do if v[cl]=="BodyGyro" or v[cl]=="BodyPosition" or v[cl]=="Hat" or v[cl]=="CharacterMesh" then v:Remove() end end 
...
function c2(wexx,smmx,c0orc1,aa,bb,cc,xx,yy,zz) q(function() c1(wexx,smmx,c0orc1,aa,bb,cc,xx,yy,zz) end) end 
...
local fhu = it("Humanoid",su) fhu.MaxHealth = 1/0 fhu.PlatformStand = true 
...
function BlastMesh(col2,pos,adjus2,sc) local adjus=adjus2 *cn(0,-0.12*sc,0) local spi=pa(m,"","Block",0,0,0,false,false,1,0,col2) spi.Anchored=true local spim=it("SpecialMesh",spi) spim.MeshType="FileMesh" spim.MeshId=asset .. Decs.Crown spim.Scale=v3(sc*1.4,(sc*0.4),sc*1.4) q(function() local fu=spim.Scale local spix=0 for i=1,0.4,-0.05 do spix=spix + 15 spi.Transparency=i spim.Scale=v3(((fu.x*i)/2)+(fu.x/2),fu.y,((fu.z*i)/2)+(fu.z/2)) spi.CFrame=pos.CFrame *adjus *ca(0,rd(spix),0) w() end for i=0.4,1,0.05 do spix=spix + 15 spi.Transparency=i spim.Scale=v3(fu.x*i,fu.y,fu.z*i) spi.CFrame=pos.CFrame *adjus *ca(0,rd(spix),0) w() end spi:Remove'' end) end 
function GlowMesh(anch,meshid2,rootz,mv3,colzz,adjus,l1,l2,l3) 
...
q(function() for i=l1,l2,l1 do spi.Transparency=i spim.Scale=mv3*i w() end spi:Remove'' end) 
end 
function Explode(onb,scz,colzz,dmge) q(function() local scale=scz*2 local scale2=scale*0.825 
local e1=pa(c,"","Block",0,0,0,false,false,0.3,0,colzz) e1.Anchored=true e1.CFrame=onb*ca(rd(-90),0,0) local e1m=it("SpecialMesh",e1) e1m.MeshType="FileMesh" e1m.MeshId=asset .. Decs.Ring 
local e2=pa(c,"","Block",0,0,0,false,false,0.3,0,colzz) e2.Anchored=true e2.CFrame=onb*ca(0,0,0) local e2m=it("SpecialMesh",e2) e2m.MeshType="FileMesh" e2m.MeshId=asset .. Decs.Ring 
local e3=pa(c,"","Block",0,0,0,false,false,0.3,0,colzz) e3.Anchored=true e3.CFrame=onb*ca(0,0,0) local e3m=it("SpecialMesh",e3) e3m.MeshType="FileMesh" e3m.MeshId=asset .. Decs.Crown 
...
local dbr=it("Model",workspace) local colz2="Really red" if ts(dmge)=="Block!" then colz2="Deep orange" end 
...
local dmgzz=dmgzz2 if game:service'Players':findFirstChild(aihu.Parent.Name) == nil then dmgzz=dmgzz*1.2 end ti(DBDH,aihu.Parent) if rn(1,a.cr[1]) == 1 then aihu:TakeDamage(dmgzz*a.cr[2]) dmgzz="Crit! " ..ts(dmgzz*a.cr[2]) else q(function() fuu(aihu.Parent) end) aihu:TakeDamage(dmgzz) end if not pos then return end splat2(pos.Position,dmgzz) q(function() w(0.08) tr(DBDH,ft(DBDH,aihu.Parent)) end) 
end) end 
...
local pro = pa(su,"","Block",2*sc,2*sc,7*sc,true,true,0.3,0,"") local prom = qi({"SpecialMesh",pro,Scale=v3(3*sc,2*sc,4*sc),VertexColor=v3(0,0,1),MeshType="FileMesh",MeshId=asset .. Decs.RedCliff,TextureId=asset .. Decs.RedCliffT}) 
...
local bal = pa(c,"","Ball",4*sc,4*sc,4*sc,false,true,0.16,0,"Really red") bal.CFrame = pos 
local spi = pa(c,"","Ball",0,0,0,false,true,0.16,0,"Deep orange") spi.CFrame = bal.CFrame local spm = qi({"SpecialMesh",spi,MeshType="FileMesh",MeshId=asset .. Decs.SpikeBall}) 
local dia = pa(bal,"","Block",0,0,0,false,true,0.3,0,"Really red") qi({"SpecialMesh",dia,Scale=v3(1*sc,2*sc,1*sc),MeshType="FileMesh",MeshId=asset .. Decs.Diamond}) 
...
local bal = pa(c,"","Ball",52,52,52,false,true,1,0,"Deep orange") bal.CFrame = pos 
for i=1,0.5,-0.1 do bal.Transparency =i wait() end 
for i=1,tms do local o = 25 
Lightning(pos.p,pos*ca(rd(i*o),rd(i*o),rd(i*o))*cn(25,0,0).p,4,2.5,"Really red",0.3,0) 
Lightning(pos.p,pos*ca(rd(i*o),rd(i*o),rd(i*o))*cn(-25,0,0).p,4,2.5,"Deep orange",0.3,0) 
Lightning(pos.p,pos*ca(rd(i*o),rd(i*o),rd(i*o))*cn(0,25,0).p,4,2.5,"Really red",0.3,0) 
Lightning(pos.p,pos*ca(rd(i*o),rd(i*o),rd(i*o))*cn(0,-25,0).p,4,2.5,"Deep orange",0.3,0) 
Lightning(pos.p,pos*ca(rd(i*o),rd(i*o),rd(i*o))*cn(0,0,25).p,4,2.5,"Really red",0.3,0) 
Lightning(pos.p,pos*ca(rd(i*o),rd(i*o),rd(i*o))*cn(0,0,-25).p,4,2.5,"Deep orange",0.3,0) 
...
local cor = db1:Clone() cor.Mesh.Scale=v3(sc,sc,sc)*5 local cor2 = db2:Clone() cor2.Mesh.Scale = cor.Mesh.Scale*1.55 qi({"Fire",cor,Color=bc('Really red').Color,SecondaryColor=bc('Deep orange').Color,Size=sc*3.5,Heat=0}) 
...
local lolpos = FindGround(cor.CFrame) if lolpos and i%rn(2,3)==0 then GlowMesh(false,asset .. Decs.Crown,t,v3(2.6*sc,0.8*sc,2.6*sc),'Navy blue',cn(lolpos.p+v3(0,0.15*sc,0)),0.1,1,0.05) end 
wait() end cor.Fire:Remove() 
q(function() 
local n1 = pa(su,"","Block",0,0,0,true,true,0.3,0,"Really red") qi({"SpecialMesh",n1,Scale=v3(3.5,3.5,8),MeshType="FileMesh",MeshId=asset .. Decs.Chakram}) 
...
local rip = pa(su,"","Block",1,1,1,false,true,0.2,0,"Deep orange") ripm = qi({"SpecialMesh",rip,MeshType="Sphere"}) 
...
local fis = pa(su,"","Block",0,0,0,false,true,0.2,0,"Dark green") fism = qi({"SpecialMesh",fis,Scale=v3(8*sc,8*sc,8*sc),MeshType="FileMesh",MeshId=asset .. Decs.Fist}) 
for i=0.5,5.5,1 do fism.Scale = v3(8*sc,((8*sc)/5.5)*i,8*sc) fis.CFrame = pos*cn(0,0,-i*sc)*ca(rd(-90),rd(-90),0) wait() end 
fism.Scale = fism.Scale+v3(1*sc,1*sc,1*sc) fis.CFrame = pos*cn(0,5*sc,-5*sc)*ca(rd(45),0,0)*ca(rd(-90),rd(-90),0) rip.CFrame=pos*ca(rd(45),0,0) qi({"Fire",fis,Heat=-25,Size=15,Color=bc('Really red').Color,SecondaryColor=bc('Deep orange').Color}) 
local arm = pa(su,"","Block",1,1,1,false,true,0.2,0,"Dark green") armm = it('CylinderMesh',arm) 
...
local worm0 = pa(mm,"","Block",0,60,0,true,true,tr,0,"Really red") qi({"SpecialMesh",worm0,Scale=v3(9*sca,12*sca,9*sca),MeshType="FileMesh",MeshId="http://www.roblox.com/asset/?id=12137531"}) 
local worm1 = pa(mm,"","Block",10*sca,6*sca,10*sca,true,true,tr,0,"Deep orange") it('CylinderMesh',worm1) 
local worm2 = pa(mm,"","Block",9.5*sca,6*sca,9.5*sca,true,true,tr,0,"Really red") it('CylinderMesh',worm2) 
local worm3 = pa(mm,"","Block",9*sca,6*sca,9*sca,true,true,tr,0,"Deep orange") it('CylinderMesh',worm3) 
local worm4 = pa(mm,"","Block",8.5*sca,6*sca,8.5*sca,true,true,tr,0,"Really red") it('CylinderMesh',worm4) 
local worm5 = pa(mm,"","Block",8*sca,6*sca,8*sca,true,true,tr,0,"Deep orange") it('CylinderMesh',worm5) 
local worm6 = pa(mm,"","Block",7.2*sca,6*sca,7.2*sca,true,true,tr,0,"Really red") qi({"SpecialMesh",worm6,Scale=v3(4*sca,18.5*sca,4*sca),MeshType="FileMesh",MeshId="http://www.roblox.com/asset/?id=1033714"}) 
for i=1,#slope-6 do if i+6 == 10 then MakeCrater(slope[i+6].p+v3(0,10,0),13,9,20) end if i+5 == 27 then local rr = 8*sca GetRegion(slope[i+5].p-v3(rr,rr,rr),slope[i+5].p+v3(rr,rr,rr),function(v) local bpp = qi({"BodyVelocity",v.Torso,maxForce=v3(1/0,1/0,1/0),velocity=cn(slope[i+5].p,v.Torso.Position+v3(0,2,0)).lookVector*120}) Dmgz(false,v.Humanoid,rn(d[1]*3.2,d[2]*4),v.Torso) wait(0.2) bpp:Remove() end,function(v) ForceBrick(v,slope[i+5].p,150) end) MakeCrater(slope[i+5].p+v3(0,10,0),13,9,20) end 
...
local tor = pa(su,"","Block",0,0,0,true,true,1,0,"Deep orange") qi({"SpecialMesh",tor,Scale=v3(6.4*sc,8*sc,6.5*sc),MeshType="FileMesh",MeshId=asset .. Decs.Tornado}) 
...
local rr = 7*sc GetRegion(tor.Position-v3(rr,rr,rr),tor.Position+v3(rr,rr,rr),function(v) if v.Torso:findFirstChild("SRP") == nil then local bp = qi({"BodyPosition",v.Torso,Name="SRP",maxForce=v3(1/0,1/0,1/0),position=tor.CFrame*cn(0,0,8*sc).p}) while tor.Parent do v.Humanoid.PlatformStand=true bp.position=tor.CFrame*cn(0,0,8*sc).p 
if rn(1,4) == 2 then Dmgz(false,v.Humanoid,rn(1,2),v.Torso) end 
wait() end bp:Remove() Dmgz(true,v.Humanoid,rn(d[1]*2,d[2]*2),v.Torso) local bpp = qi({"BodyVelocity",v.Torso,maxForce=v3(1/0,1/0,1/0),velocity=cn(tpos,v.Torso.Position+v3(0,2,0)).lookVector*100}) wait(0.35) bpp:Remove() v.Humanoid.PlatformStand = false else end end) 
end end) 
wait(tyms) tpos = tor.Position local torm = tor.Mesh.Scale tor:Remove() GlowMesh(false,asset .. Decs.Tornado,t,torm*3,'Dark green',cn(tpos),0.2,1,0.05) 
end) end
function CrownSwords(pos) q(function()
local hol = pa(su,"","Block",1,1,1,false,true,0.1,0,"Really red") qi({"SpecialMesh",hol,MeshType="Sphere"})
...
local swr = {} for i=1,14 do local swor = pa(su,"","Block",0,0,0,false,true,0.2,0,"Deep orange") qi({"SpecialMesh",swor,MeshType="FileMesh",Scale=v3(4,4,4),MeshId=Decs.SplinteredSky,TextureId=Decs.SplinteredSkyT,VertexColor=v3(0,0,0.8)}) ti(swr,swor)  end
local swr2 ={} for i=1,8 do local swor = pa(su,"","Block",0,0,0,false,true,0.2,0,"Really red") qi({"SpecialMesh",swor,MeshType="FileMesh",Scale=v3(4,4,4),MeshId=Decs.SplinteredSky,TextureId=Decs.SplinteredSkyT,VertexColor=v3(0,0,0.8)}) ti(swr2,swor)  end
local swor = pa(su,"","Block",0,0,0,false,true,0.2,0,"Deep orange") qi({"SpecialMesh",swor,MeshType="FileMesh",Scale=v3(10,14,7.5),MeshId=Decs.SplinteredSkyT,TextureId=Decs.SplinteredSkyT,VertexColor=v3(0,0,0.8)})
...
local bl = pa(su,"","Block",9.5*sc,12*sc,4*sc,true,true,0.05,0,"Deep orange") local rca = ca(rd(rn(-6,6)),rd(rn(-6,6)),rd(rn(-6,6)))
local sk = pa(su,"","Block",0,0,0,false,true,0.05,0,"Really red") qi({"SpecialMesh",sk,Scale=v3(2*sc,2*sc,0.8*sc),MeshType="FileMesh",MeshId=Decs.Skull1})
...
GlowMesh(false,"Sphere",t,v3(12,12,12),'Deep orange',v.CFrame,0.1,1,0.1)
...
wait(rn(23,30))
for i,v in pairs({{"Navy blue",v3(30,30,30)},{"Really red",v3(20,20,20)}}) do  GlowMesh(false,"Sphere",t,v[2],v[1],bl.CFrame,0.1,1,0.1) end
...
---- TUT01LOL 
han = pa(su,"","Block",0.4,0.4,0.4,false,false,0,0,"Deep orange") it('CylinderMesh',han) 
hanw = qi({"Weld",han,Part0=t,Part1=han,C0=cn(0.7,-0.7,0.65)*ca(0,0,rd(45))*ca(0,rd(90),0)}) 
han2 = pa(su,"","Block",0.35,4,0.35,false,false,0,0,"Really red") it('CylinderMesh',han2) 
han2w = qi({"Weld",han2,Part0=han,Part1=han2,C0=cn(0,1,0)}) 
han3 = pa(su,"","Block",0,0,0,false,false,0,0,"Deep orange") qi({"SpecialMesh",han3,MeshType="Sphere",Scale=v3(0.4,0.4,0.4)*5}) 
...
for i=1,7 do 
des = pa(su,"","Block",0.4,0.2,0.4,false,false,0,0,"Really red") it('CylinderMesh',des).Scale = v3(1,0.4,1) 
desw = qi({"Weld",han,Part0=han2,Part1=des,C0=cn(0,-0.9+(i/3),0)*ca(rd(rn(-15,15)),0,rd(rn(-15,15)))}) 
end 
for i=0,270,90 do 
des = pa(su,"","Block",0,0,0,false,false,0,0,"") qi({"SpecialMesh",des,MeshType="FileMesh",Scale=v3(0.1,0.4,0.1),MeshId=asset .. Decs.Spike}) 
desw = qi({"Weld",han2,Part0=han2,Part1=des,C0=ca(0,rd(i),0)*cn(0,1.8,0.3)*ca(rd(90),0,0)}) 
end 
han5 = pa(su,"","Block",0,0,0,false,false,0,0,"Deep orange") qi({"SpecialMesh",han5,MeshType="Sphere",Scale=v3(0.35,0.6,0.35)*5}) 
han5w = qi({"Weld",han2,Part0=han2,Part1=han5,C0=cn(0,2,0)}) 
han6 = pa(su,"","Wedge",0,0,0,false,false,0,0,"Really red") qi({"SpecialMesh",han6,MeshType="Wedge",Scale=v3(0.1,1,0.5)*5}) 
han6w = qi({"Weld",han2,Part0=han2,Part1=han6,C0=cn(0,2.4,0.05)*ca(rd(10),0,0)}) 
han7 = pa(su,"","Block",0,0,0,false,false,0,0,"") qi({"SpecialMesh",han7,MeshType="FileMesh",Scale=v3(0.75,0.75,2),MeshId=asset .. Decs.Ring}) 
han7w = qi({"Weld",han2,Part0=han2,Part1=han7,C0=cn(0,2.9,0)*ca(0,rd(90),0)}) 
han8 = pa(su,"","Block",0,0,0,false,false,0.3,0.1,"Really red") qi({"SpecialMesh",han8,MeshType="Sphere",Scale=v3(0.7,0.7,0.7)*5}) 
han8w = qi({"Weld",han2,Part0=han2,Part1=han8,C0=cn(0,2.9,0)*ca(0,0,0)}) 
han9 = pa(su,"","Block",0,0,0,false,false,0,0,"Deep orange") qi({"SpecialMesh",han9,Scale=v3(0.25,1,0.25),MeshType="FileMesh",MeshId = asset .. Decs.Chakram3}) 
...
for i=0,2 do 
des = pa(su,"","Block",0,0,0,false,false,0,0.05,"Really red") qi({"SpecialMesh",des,MeshType="FileMesh",Scale=v3(0.2,1,0.2),MeshId=asset .. Decs.Diamond}) 
desw = qi({"Weld",han2,Part0=han2,Part1=des,C0=cn(0,2.1+(i/2.1),0.6+(i/5))*ca(rd(150-(i*30)),0,0)}) 
end 
des = pa(su,"","Block",0,0,0,false,false,0,0.05,"Deep orange") qi({"SpecialMesh",des,MeshType="FileMesh",Scale=v3(0.2,1,0.2),MeshId=asset .. Decs.Diamond}) 
...
han10 = pa(su,"","Block",0,0,0,false,false,0,1,"Really red") it('CylinderMesh',han10).Scale = v3(1.2,0.57,1.2) 
...
raa = c["Right Arm"] 
laa = c["Left Arm"] 
clawa1 = pa(su,"","Block",0,0,0,false,false,0,0,"Really red") qi({"SpecialMesh",clawa1,MeshType="FileMesh",Scale=v3(0.12,0.8,0.12),MeshId=asset .. Decs.Spike}) 
clawa1w = qi({"Weld",han2,Part0=laa,Part1=clawa1,C0=ca(rd(180),0,0)*cn(-0.2,1.25,0)}) 
clawa2 = pa(su,"","Block",0,0,0,false,false,0,0,"Deep orange") qi({"SpecialMesh",clawa2,MeshType="FileMesh",Scale=v3(0.12,0.8,0.12),MeshId=asset .. Decs.Spike}) 
clawa2w = qi({"Weld",han2,Part0=laa,Part1=clawa2,C0=ca(rd(180),0,0)*cn(0.2,1.25,0)}) 
clawb1 = pa(su,"","Block",0,0,0,false,false,0,0.4,"Really red") qi({"SpecialMesh",clawb1,MeshType="FileMesh",Scale=v3(0.12,0.8,0.12),MeshId=asset .. Decs.Spike}) 
clawb1w = qi({"Weld",han2,Part0=clawa1,Part1=clawb1,C0=cn(0,0.15,0)}) 
clawb2 = pa(su,"","Block",0,0,0,false,false,0,0.4,"Really red") qi({"SpecialMesh",clawb2,MeshType="FileMesh",Scale=v3(0.12,0.8,0.12),MeshId=asset .. Decs.Spike}) 
clawb2w = qi({"Weld",han2,Part0=clawa2,Part1=clawb2,C0=cn(0,0.15,0)}) 
badge = pa(su,"","Block",0,0,0,false,false,0,0,"Really red") qi({"SpecialMesh",badge,MeshType="FileMesh",Scale=v3(0.15,1,0.5),MeshId=asset .. Decs.Diamond}) 
badgew = qi({"Weld",han2,Part0=laa,Part1=badge,C0=cn(-0.5,0,0)*ca(rd(25),0,0)}) 
badge2 = pa(su,"","Block",0,0,0,false,false,0,0,"Really red") qi({"SpecialMesh",badge2,MeshType="FileMesh",Scale=v3(0.149,1,0.495),MeshId=asset .. Decs.Diamond}) 
badge2w = qi({"Weld",han2,Part0=badge,Part1=badge2}) 
ef = pa(su,"","Block",0,0,0,false,true,1,0,"Really red") qi({"SpecialMesh",ef,MeshType="FileMesh",Scale=v3(7,7,4),MeshId=asset .. Decs.Chakram2}) 
--efw = qi({"Weld",han2,Part0=t,Part1=ef,C0=cn(0,-2.9,0)*ca(rd(90),0,0)}) 
db1 = pa(su,"","Block",0,0,0,false,false,1,0,"Deep orange") qi({"SpecialMesh",db1,MeshType="Sphere",Scale=v3(1,1,1)*5}) 
db1w = qi({"Weld",han2,Part0=laa,Part1=db1,C0=cn(0,-2,0)}) 
db2 = pa(su,"","Block",0,0,0,false,false,1,0.1,"Really red") qi({"SpecialMesh",db2,MeshType="Sphere",Scale=v3(1.2,1.2,1.2)*5}) 
...
sho = pa(su,"","Block",0,0,0,false,false,0,0,"Deep orange") qi({"SpecialMesh",sho,Scale=v3(1.1,1.1,1.1)*5,MeshType="Wedge"}) 
show = qi({"Weld",han2,Part0=raa,Part1=sho,C0=cn(0,0.545,0)*ca(rd(180),rd(90),0)}) 
chak = pa(su,"","Block",0,0,0,false,false,0,0,"") qi({"SpecialMesh",chak,Scale=v3(0.09,0.09,0.24)*5,MeshType="FileMesh",MeshId=asset .. Decs.Chakram}) 
chakw = qi({"Weld",han2,Part0=raa,Part1=chak,C0=cn(0.56,0.5,0)*ca(0,rd(-90),0)}) 
chak2 = pa(su,"","Block",0,0,0,false,false,0.1,0.1,"Really red") qi({"SpecialMesh",chak2,Scale=v3(0.15,0.15,0.05)*5,MeshType="FileMesh",MeshId=asset .. Decs.Skull1}) 
...
q(function() while true do wait(0.035) --[[efw.C0 = efw.C0*ca(0,0,rd(25))]] chak2w.C0 = chak2w.C0*ca(0,0,rd(25)) end end) 
---- TUT02LOL 
function Act(k,kb) 
if kb then 
if k == "e" and Anim=="None" then 
Anim = "SpawnSwords" Armz(false) e() eb(raa) 
c2(law,spd,3,-1.5,1,0,30,0,30) c2(tw,spd,3,0,0,0,0,-95,0) c2(hw,spd,3,0,1.5,0,0,95,0) 
c1(raw,spd,3,1,0.5,0,0,0,-80) c2(raw,spd,3,1,0.4,0,0,0,-140) for i=1,5 do SpawnSword(FindGround(to.CFrame*cn(0,-2.75,-0.5+(-i*8)-i)),1+(i/10),15,"Really red") wait(0.06) end 
eb(raa) wait(0.2) e() c2(law,spd,3,-1.5,1,0,0,0,0) c2(hw,spd,3,0,1.5,0,0,0,0) c2(tw,spd,3,0,0,0,0,0,0) c1(raw,spd,3,1.5,1,0,0,0,0) Armz(true) 
Anim = "None" 
...
Trail(bla3,15,0,"Really red",0.65,cn(0,-0.2,-1)) rayHit(0.6,1,ra,ca(rd(-90),0,0),8,1.3) 
...
block.Value = true repeat wait() until not ky[k] for i=11,1,-1 do chak2.Reflectance = i/40 chak2.Mesh.Scale = spm*i wait() end chak2.Reflectance = 0 
...
Anim = "FistBoom" Armz(false) fac = true 
...
c2(hanw,spd*1.5,0,-0.1,-1.2,0,0,-30,100) c1(raw,spd*1.5,3,1.5,0.5,0,-135,-60,0) wait(0.2) FistBoom(cn(to.CFrame*cn(-1.5,1.5,-4).p,to.CFrame*cn(-1.5,1.5,-5).p),1,rn(d[1]*2,d[2]*2),(to.Position-v3(mouse.hit.p.x,to.Position.y,mouse.hit.p.z)).magnitude-10) fac = false c1(raw,spd,3,1,0.5,0,-50,70,0) 
wait(0.2) c2(hanw,spd*0.7,0,-0.1,-1.2,0,0,0,70) c1(raw,spd,3,1.5,0.5,0,-340/2,0,40) c1(raw,spd,3,1.5,0.5,0,-340,0,25) hanw.Part0 = t hanw.C1=ca(0,0,0) hanw.C0=cn(0.7,-0.7,0.65)*ca(0,0,rd(45))*ca(0,rd(90),0) 
...
local tpos = mouse.hit.p if (tpos - bla2.Position).magnitude < 2000 then q(function() local to = pa(su,"","Block",2,1,2,false,true,0.1,0,"Deep orange") it('CylinderMesh',to) local to2 = pa(su,"","Block",0,0,0,false,true,0.1,0,"") qi({"SpecialMesh",to2,Scale=v3(2,2,2),MeshType="FileMesh",MeshId=asset .. Decs.Totem}) 
...
for i=1,8 do Lightning(bla2.Position,tpos,math.floor((bla2.Position-tpos).magnitude/18)+1,1.5,"Deep orange",0.3,0) wait(0.12) end fac = false 
caw = true q(function() wait(1) WormDrill(cn(tpos),rn(25,30)/10) end) c1(law,spd,3,-1.5,0.25,0,0,0,180) q(function() local to = pa(su,"","Block",1.5,1.5,1.5,false,true,0.1,0,"Really red") qi({"SpecialMesh",to,MeshType="Sphere"}) for i=1,15,1 do to.Size=v3(1.5,i*2,1.5) to.CFrame=la.CFrame*cn(0,-2+(-i),0) wait() end GlowMesh(false,"Sphere",t,v3(35,35,35),'Really red',la.CFrame*cn(0,-32,0),0.1,1,0.1) fade(to,0.1) end) 
...
c2(raw,spd,3,1,0.5,-0.5,-70,-25,0) c2(law,spd,3,-1,0.5,-0.5,-70,25,0)  c1(tw,spd,3,0,0,0,30,0,0) wait(0.2) GlowMesh(false,"Sphere",t,v3(9,15,12),'Really red',to.CFrame,0.1,1,0.05) CrownSwords(to.CFrame*cn(0,-2.5,-3)) to.CFrame = to.CFrame*cn(0,0,30) GlowMesh(false,"Sphere",t,v3(9,15,12),'Really red',to.CFrame,0.1,1,0.05) 
...
c2(raw,spd*1.4,3,1,0.5,-0.5,-45,-25,0) c1(law,spd*1.4,3,-1,0.5,-0.5,-45,25,0) GlowMesh(false,"Sphere",t,v3(15,1,15),'Deep orange',to.CFrame*cn(0,-2.8,-5.5),0.1,1,0.03)
...
local pla = pa(su,"","Block",7,1,7,true,true,1,0,"Really red")
function ButtonDown() if Anim~="None" then return end 
to.Velocity=v3(0,0,0) if hu.Jump then GlowMesh(false,"Sphere",t,v3(8,2,8),'Deep orange',to.CFrame*cn(0,-2,0),0.1,1,0.05)  GlowMesh(false,Decs.Crown,t,v3(5,3,5),'Really red',to.CFrame*cn(0,-1.5,0)*ca(rd(180),0,0),0.1,1,0.05) to.CFrame=to.CFrame*cn(0,20,0) 
hu.Jump=false GlowMesh(false,"Sphere",t,v3(8,2,8),'Really red',to.CFrame*cn(0,-4,0),0.1,1,0.05)
else GlowMesh(false,"Sphere",t,v3(8,10,2),'Deep orange',to.CFrame,0.1,1,0.05)
GlowMesh(false,Decs.Crown,t,v3(5,3,5),'Really red',to.CFrame*cn(0,0,1)*ca(rd(90),0,0),0.1,1,0.05) to.CFrame=to.CFrame*cn(0,0,-12) GlowMesh(false,"Sphere",t,v3(8,10,2),'Really red',to.CFrame*cn(0,0,1),0.1,1,0.05) end
pla.CFrame= to.CFrame*cn(0,-3.4,0) local orpos = pla.Position repeat wait(0.5) GlowMesh(false,"Sphere",t,v3(14,1,14),'Deep orange',cn(orpos)*cn(0,0.5,0),0.1,1,0.02) until pla.Position~=orpos or (pla.Position-to.Position).magnitude>10
