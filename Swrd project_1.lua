...
function spikes3(part,color)
	local hah = math.random(35,80)/10
	local p = Instance.new("Part",sword)
	prop(p,me.Character,false,0.4,0,0.2,0.2,0.2,color,true,"Custom")
	p.CFrame = part.CFrame * CFrame.new(0,0,-4) * CFrame.Angles(math.pi/2,0,0)
	local mww = Instance.new("SpecialMesh")
	mesh(mww,p,hah,hah*1.3,hah,"FileMesh")
	mww.MeshId = "http://www.roblox.com/asset/?id=20329976"
	local lol = math.random(5,9)/100
	local klol =  math.random(44,88)/100
	fade(p,mww,klol,p.Transparency,lol)
end

function balls(part,color,dist)
	local p = Instance.new("Part",sword)
...
function kill(hit,mod)
	if deb and attack and hit.Parent.Name ~= name then
		...
		if ch:findFirstChild("Humanoid") and ch:findFirstChild("Head") and ch:findFirstChild("Torso") then
			...
			if cripro == 1 then
				cri = true
				dm = math.random(avgdmg*1.5,avgdmg*4)
			end
			ch["Humanoid"].Health = ch["Humanoid"].Health - dm
			blast(dm,ch.Head,cri)
			if cri then
				coroutine.resume(coroutine.create(function()
					if ch.Torso:GetMass() < 45 then
					...
					if mod == true then
						local lo = ch.Torso
						lo.RotVelocity = Vector3.new(math.random(-10,10),math.random(-10,10),math.random(-10,10))
						lo.Velocity = Vector3.new(math.random(-10,10),math.random(6,30),math.random(-10,10))
					end
					end
					end
				end))
...
function att()
	...
	for i=1,6 do
		...
		block(blade5,3.3,{"Medium blue", "Light blue", "White", "Tr. Blue"})
		wait()
	end
	attack = false
	wait(0.08)
...
function spin()
	...
	for i=1, 30 do
		...
		block(blade5,1.5,{"Medium blue", "Light blue", "White", "Tr. Blue"})
	end
	bv:remove()
	bav:remove()
...
function sprint()
	...
	coroutine.resume(coroutine.create(function()
		for i=1, 12 do
			wait()
			balls(blade5,"Medium blue",splashdist*2)
		end
	end))
	local parts = getcharparts(workspace)
	for _,v in pairs(parts) do
		if (v.Position - blade5.Position).magnitude < splashdist then
			kill(v,true)
		end
	end
	attack = false
...
function rush()
	...
	hum.WalkSpeed = 0
	pos = torso.CFrame
	pos2 = pos * CFrame.new(0,0,-6)
	avgdmg = normdmg*0.6
	dela = 0.2
	for i=1,8 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(-0.05,0.1,0.1) * CFrame.new(0,0,0)
		weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(0.07,0,0) * CFrame.new(0,0,0)
		neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(-0.07,0,0)
		wait()
	end
	charge:play()
	wait(0.1)
	...
	local bv = Instance.new("BodyVelocity",torso)
	bv.maxForce = Vector3.new(math.huge,math.huge,math.huge)
	bv.velocity = torso.CFrame.lookVector * 43
	local bg = Instance.new("BodyGyro",torso)
	...
	for i=1, 32 do
		wait()
		spikes3(torso,"Medium blue")
		block(blade2,4,{"Medium blue", "Light blue", "White", "Tr. Blue"})
	end
	bv:remove()
	bg:remove()
	criticalchance = normchance
...
function stab()
	...
	dela = 0.2
	for i=1,10 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(-0.18,0,-0.04)
		lw.C0 = lw.C0 * CFrame.fromEulerAnglesXYZ(0.04,0,0.1)
		weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(0.08,0,0.02) * CFrame.new(0,0,0)
		neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(0,0,0)
		wait()
	end
	wait(0.1)
	attack = true
	for i=1,7 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(0.34,0,-0.06)
		lw.C0 = lw.C0 * CFrame.fromEulerAnglesXYZ(0.12,0,-0.03)
		weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(-0.2,0,0.02) * CFrame.new(0,0,0)
		neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(0,0,0)
		block(blade5,1,{"Medium blue", "Light blue", "White", "Tr. Blue"})
		block(blade2,1.5,{"Medium blue", "Light blue", "White", "Tr. Blue"})
		wait()
	end
	attack = false
	for i=1,7 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(-0.04,0.1,0.07)
		lw.C0 = lw.C0 * CFrame.fromEulerAnglesXYZ(-0.12,0,-0.12)
		weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(0.05,0,0.02) * CFrame.new(0,0,0)
		neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(0,0,0)
		wait()
	end
	dela = 0.4
...
function buff()
	...
	attacking = true
	for i=1,10 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(0.18,0,0)
		wait()
	end
	wait(0.1)
	for i=1,7 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(0.1,0,-0.06)
		weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(-0.2,0,0.02) * CFrame.new(0,0,0)
		spikes(torso,"Medium blue")
		block(torso,1.5,{"Medium blue", "Light blue", "White", "Tr. Blue"})
		wait()
	end
	coroutine.resume(coroutine.create(function()
		normdmg = nornor*2
		...
		for i=1, 65 do
			wait(0.1)
			block(main,1,{"Medium blue", "Light blue", "White", "Tr. Blue"})
		end
		normdmg = nornor
		avgdmg = normdmg
	end))
	for i=1,7 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(-0.3,0,0.02)
		weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(0.2,0,-0.02)
		wait()
	end
	rw.C0 = rightfight
	lw.C0 = leftfight
	...
	while selected do
		...
		if k == 1 then
			...
			for i=1, math.random(4,22) do
				...
				bg.maxForce = Vector3.new(40000,40000,40000)
				bg.position = torso.Position
				coroutine.resume(coroutine.create(function()
					repeat
						bg.position = torso.Position
						wait()
					until (p.Position - torso.Position).magnitude < 2.4
					hum.Health = hum.Health + 1.6
					p:remove()
				end))
			end
			end
