...
--	Primary Development ended on 7/16/2016
...
coroutine.resume(coroutine.create(function()
	while Snowball do
		...
		if not asd and asd.Mesh and asd.Mesh.Parent then break end
			asd.Mesh.Scale = asd.Mesh.Scale + Vector3.new(.05,.05,.05)
			asd.Transparency = i
...
SMesh.Scale = Vector3.new(.85, .3,.1)
...
SMesh2.Scale = Vector3.new(.55, .3,.1)
...
Ball.TopSurface,Ball.BottomSurface = 0,0
Ball.Locked = true
local BMesh2 = Instance.new('SpecialMesh', Ball)
BMesh2.MeshType = 'Sphere'
BMesh2.Scale = Vector3.new(.6,.6,.6)
...
Weld.Part0 = Body
Weld.Part1 = Stick
Weld.C0 = CFrame.new(0, .3, 0) * CFrame.Angles(math.rad(10),math.rad(20),0)

local Weld2 = Instance.new('Weld', Totem)
Weld2.Name = 'Stick2Weld'
Weld2.Part0 = Body
Weld2.Part1 = Stick2
Weld2.C0 = CFrame.new(0, -.2, 0) * CFrame.Angles(math.rad(10),-math.rad(20),0)
...
local coro = coroutine.resume(coroutine.create(function()
	while needaloop do
	wait() --game:service'RunService'.RenderStepped:wait()
		local c = char
		local radius = 10
...
coroutine.wrap(function()
	while Body do
		pos = pos * CFrame.Angles(math.rad(math.sin(tick())*.2),math.rad(math.sin(tick())*.2),math.rad(math.sin(tick())*.2))
		Weld.C0 = Weld.C0 * CFrame.Angles(-math.rad(math.sin(tick())*.6),0,math.rad(math.sin(tick())*.6))
		Weld2.C0 = Weld2.C0 * CFrame.Angles(math.rad(math.sin(tick())*.6),0,-math.rad(math.sin(tick())*.6))
		Body.CFrame = pos
		game:service'RunService'.RenderStepped:wait()
	end
end)()
...
for i = 0,15 do
	...
	Orbi.Parent = char
	ypcall(function()
		for i = .6,1,.02 do wait(.05)
			OrbiMesh.Scale = OrbiMesh.Scale + Vector3.new(.3, .3, .3)
			Orbi.Transparency = i
...
function Shadowmeld()
	...
	coroutine.wrap(function()
	wait(15)
	if Stealthed then Shadowmeld() end
	end)()
	end
end
...
function healHuman(hum,d)
	...
	if not Enemy[hum] then
		...
		local twenty = hum.MaxHealth * .25
		if hum.Health <= twenty then d = d * 1.6 end
		hum.Health = hum.Health - math.ceil(d/2)---:TakeDamage((math.ceil(d))/2)
...
pcall(function() hum.Parent.Health.Disabled = true end)
	...
	hum.Health = hum.Health - math.ceil(d)--:TakeDamage((math.ceil(d)))
	if hum.Health <= 0 then logDamage('Kills', 1)  end
end

------ DOTS ------
function dot:VoidDrain()

if not currentTarget then return end
if not shadowform then return end
if currentTarget==hum then return end

local ct = currentTarget

if ct:findFirstChild(plr.Name..'void')then return end
...
for i = 40, 0,-1 do
damageHuman(ct, getDPS()*.25, Object)
wait(.2)
end
debouncing = false
Voided:Destroy()
...
if ct:findFirstChild(plr.Name..'pain')then return end
...
function CreateUI(Player)
	...
	local fStatus = Create("TextLabel"){
	    ...
	    TextYAlignment = Enum.TextYAlignment.Center;
	    TextScaled = false;
	    BackgroundColor3 = Color3.new(50/255, 50/255, 170/255);
	}
	...
	local Flashheal = Create("TextLabel"){
	    Visible = true;
	...
	local HealingTouch = Create("TextLabel"){
	    Visible = true;
	    FontSize = Enum.FontSize.Size14;
	    Active = false;
	    Parent = CoolDowns;
	...
	local Shadowmeld = Create("TextLabel"){
	    Visible = true;
	    FontSize = Enum.FontSize.Size14;
	    Active = false;
	    Parent = CoolDowns;
	...
	local Line7 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[V]  -  [Totem]: Places a Healing Stream Totem";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line6 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[E  -  [Healing Touch]: Instant heal on current target.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line5 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[Q]  -  [Flash Heal]: Heals target over time.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line4 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[G]  -  [Target Toggle]: Toggles enemy/ally on your current target.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line3 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[Y]  -  [Retreat]: Brings your target back to yourself.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line1 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[Ctrl+Jump]: Teleportation forward";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	    Position = UDim2.new(0, 0, 0, 0);
	...
	local Line8 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[Z]  -  [Shadowmeld]: Hides your character.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line9 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[X]  -  [Shield]: Shields you from damage and heals/damages nearby players.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line10 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[R]  -  [ShadowSight]: Monitor your target";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line11 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[Q]  -  [Shadow Word:Pain]: Deals large amount of damage over time. (Requires Shadow Form)";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line12 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[E]  -  [Void Drain]: Deals immense damage in a quick period of time. (Requires Shadow Form)";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line13 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "[ALT]  -  [Shadowform]: Allows for a wider range of shadow attacks in place of holy.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	local Line14 = Create("TextLabel"){
	    ...
	    ClipsDescendants = false;
	    BorderColor3 = Color3.new(0.105882, 0.164706, 0.207843);
	    Text = "This class will provide a passive 60% damage boost to any melee or caster.";
	    TextColor3 = Color3.new(1, 1, 1);
	    Font = Enum.Font.SourceSans;
	    BackgroundTransparency = 1;
	...
	function this:SetCT(int)--//Casting
...
if checkCooldown'Shadowform' then return end
setCooldown('Shadowform', 5)

if not shadowform then
if stats['Shards'] < 6 then return end
shadowform = true
stats['Shards'] = 0
local cParts = {}

function getAllParts(q)
...
for no, prt in next,cParts do
	if prt~=rootpart then
		prt.Transparency = .1
	end
end

for asd, dsa in next,char:children'' do
...
coroutine.wrap(function() 
	...
	if shadowform then shadowForm() end
end)()
...
if not currentTarget then Targ = hum else Targ = currentTarget end
	...
	if isEnemy() then target.Name = 'Enemy' else target.Name = 'Ally' end
	...
	if Targ:FindFirstChild('FlashHeal') then 
		ms = 'Toothpaste'
		ss = 'White'
	elseif Targ:FindFirstChild('HealingTouch') then
		ms = 'Bright violet'
		ss = 'Lime green'
	else 
...
if Enemy[currentTarget] then return true end
...
pcall(function()if Enemy[currentTarget] then  Enemy[currentTarget] = nil  else Enemy[currentTarget] = true end end)
...
Character.Humanoid.Jumping:connect(function()
	...
	if hum.WalkSpeed < 8 then return end
	if Ctrl then
		if not checkCooldown('Teleport') then
		if not useMana(25) then return end
		setCooldown('Teleport', 2)
...
if (tick()-lastdps < 10) then return end
regening = true
	local s = wait(1)
...
function KeyDown(key)
	...
	if key ~= 't' and key ~= 'r' and key ~= 'g' and (key ~= 'x' and shielding) and otheranims then return end
	if (key == 'q' or key == 'e' or key == 'f' or key == 'x' or key == 'b' or key:byte() == 52) and Stealthed then Shadowmeld() end
	...
	shadowForm()
	elseif key == 'z' then
	if checkCooldown'Shadowmeld' then return end
	if not useMana(35) then return end
	setCooldown('Shadowmeld',15)
	Shadowmeld()
	randomCastSound()
	elseif key == 'b' then
	if checkCooldown'Totem' then return end
	if not useMana(45) then return end
	...
	wait(.3)
	otheranims = false
	elseif key == 'e' and shadowform then
	if checkCooldown'VoidDrain' then return end
	...
	wait(.3)
	otheranims = false
	elseif key == 'q' and shadowform then
	if checkCooldown'SWP' then return end
	setCooldown('SWP',2)
	otheranims = true
	Mode = 'Casting2'
	coroutine.wrap(function()
	...
	otheranims = false
	elseif key == 'x' then
	if shielding then shielding = false return end
	...
	sm.Scale = Vector3.new(10,2,10)
		while wait() do
		if not useMana(5) then break end
		if not shielding then break end
		s.CFrame = s.CFrame * CFrame.fromEulerAnglesXYZ(0,6,0)
		local f = Instance.new("Part",Character)
		f.BrickColor = (math.random(1,10)<6 and BrickColor.new('Toothpaste') or BrickColor.new('White'))
		f.Size = Vector3.new(5,1,5)
		f.Anchored = true
		f.Locked = true
		f.CanCollide = false
		...
		Wind.Locked = true
		Wind.BrickColor = (math.random(1,10)<6 and BrickColor.new('Toothpaste') or BrickColor.new('White'))
	...
	elseif key == 'v' then
		otheranims = true
		Mode = 'Swipe'
		if not isEnemy() then giveBuff('Power Word:Fortitude', 'MainStat', '60', '3') end
		wait(.5)
		otheranims = false
	elseif key == 'y' then
		currentTarget = hum
	...
	--otheranims = true
	--Mode = 'Casting1'
	elseif key == 'e' and not shadowform then
	if checkCooldown'HealingTouch' then return end
	if checkRange() then return end
	if not useMana(25) then return end
	...
	elseif key == 'f' then
	if checkCooldown'Burst' then return end
	if checkRange() then return end
	if not useMana(15) then return end
	setCooldown('Burst', 3)
	otheranims = true
	Mode = 'Casting2'
	expandDong()
	...
	wait(.5)
	otheranims = false
	elseif key == 'q' and not shadowform then
	if checkCooldown'FlashHeal' then return end
	if cTarg:FindFirstChild('FlashHeal') then return end
	if checkRange() then return end
	if not useMana(20) then return end
	setCooldown('FlashHeal', 3)
	local Object
	if not cTarg then Object = torso else Object = cTarg.Parent.Torso end
	otheranims = true
	Mode = 'Casting2'
	if cTarg:findFirstChild'FlashHeal' then return end
	...
	coroutine.wrap(function()
	for i = 25,0,-1 do
	healHuman(cTarg or hum, (healCalc*.4))
	logDamage('FlashHeal', healCalc*.4)
	wait(.4)
	end
	FlashHeal:Destroy()
	debouncing = false
...
recount()
elseif m=='debugmode' and (plr.Name=='Darkus_Theory' or plr.Name == 'worthystone' or plr.Name == 'coty645' or plr.Name == 'lsaiah328') then
if cheating then
cheating = false
stats['Haste'] = hst
stats['Intellect'] = str
...
coroutine.wrap(function()
		...
		if stats["Shards"] > 6 then stats["Shards"] = 6 end
		if stats["Shards"] < 0 then stats["Shards"] = 0 end
		if stats["Mana"] > 500 then stats["Mana"] = 500 end
		if stats["Mana"] < 0 then stats["Mana"] = 0 end
		if stats["Mana"] < 500 and stats['Mana'] > 200 then stats["Mana"] = stats["Mana"] + .25 elseif stats['Mana'] < 200 then stats['Mana'] = stats['Mana'] + .4 end
		...
		if not (btimer and btimer > 0) then if hum.WalkSpeed == 0 then UI:SetStatus('Stunned') elseif hum.WalkSpeed < 10 then UI:SetStatus('Slowed') else UI:SetStatus('Normal') end else UI:SetStatus('Shadowform ['..btimer..']') hum.WalkSpeed = 30 end
		...
		local mg = Vector3.new(torso.Velocity.x, 0, torso.Velocity.z).magnitude
			if not otheranims then
				...
				Mode = "Idling"
				elseif (mg > 3 and mg < 21) and not Jumping then
				Mode = "Walking"
				elseif (mg > 21) and not Jumping then
				Mode = "Running"
				elseif Jumping and (Vector3.new(0, torso.Velocity.y, 0).magnitude > 2) and (mg > 1) then
				Mode = "Jumping"
				end
			end
		count = (count % 100) + (1)*(hum.WalkSpeed)/(5*1.3)
		torso = char:FindFirstChild('Torso')
		...
		elseif Mode == 'Idling' then 
				lerp(rlegm, rlegm.C0, rlc0 * CFrame.Angles(0, 0, math.rad(5)),.3)
				lerp(llegm, llegm.C0, llc0 * CFrame.Angles(0, 0, -math.rad(5)),.3)
				lerp(neck, neck.C0, neckc0 * CFrame.Angles(math.rad(5), 0, 0),.3)
				lerp(lm, lm.C0, lsc0 * CFrame.Angles(0, 0, -math.rad(10)+(angle*0.0200)),.1)
				lerp(rm, rm.C0, rsc0 * CFrame.Angles(math.rad(90), 0, math.rad(90)),.3)
		...
		elseif Mode == 'Casting1' then
				...
				lerp(lm, lm.C0, lsc0 * CFrame.new(.2,0,0) * CFrame.Angles(math.rad(45), math.rad(35), math.rad(60)),.3)
				lerp(rm, rm.C0, rsc0 * CFrame.Angles(math.rad(45), math.rad(35), math.rad(70)),.3)
				lerp(rj, rj.C0, rootc0 * CFrame.Angles(0,0,math.rad(40)),.3)
		elseif Mode == 'Casting2' then
				lerp(rlegm, rlegm.C0, rlc0 * CFrame.Angles(0, -math.rad(20), math.rad(3)),.3)
				lerp(llegm, llegm.C0, llc0 * CFrame.Angles(0, 0, -math.rad(3)),.3)
				lerp(neck, neck.C0, neckc0 * CFrame.Angles(0, 0, -math.rad(30)),.3)
				lerp(lm, lm.C0, lsc0 * CFrame.Angles(0, 0, -math.rad(25)),.3)
				lerp(rm, rm.C0, rsc0 * CFrame.Angles(math.rad(20), math.rad(55), math.rad(80)),.3)
				lerp(rj, rj.C0, rootc0 * CFrame.Angles(0,0,math.rad(40)),.3)
