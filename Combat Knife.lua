...
Your everyday combat knife. Serrated edge for maximum cutting power.
Script by DarkShadow6

Changelog:
 o 1.0
    o Edited throwing capability:
       o Now it plays sounds when spinning and hitting.
       o It sticks to the person you throw it at (or the ground if you fail at aiming).
       o It doesn't randomly break anymore (usually).
       o It doesn't fall apart when hitting a player and killing him/her.
       o Added hit-specific damage (headshot is instant kill).
 o 0.99
    o Edited throwing capability. The knife spins!
 o 0.98
    o Edited throwing capability. Now you can pick your knife back up!
 o 0.97
    o Edited throwing capability. Now it does damage!
 o 0.95
    o Press E to switch between Stab (instakill/slower) and Slash (less damage/faster).
 o 0.91
    o Added throwing capability. Press Z to use.
 o 0.1 - 0.9
    o Added animation.
    o Made a model.
--]]
...
function onKeyDown(key, mouse)
	...
	if key == "z" and Button1Down == false and canUse == true then
		...
		Velocity.velocity = (mouse.Hit.p - Player.Character[Name].Handle.Position).unit * 100
		...
		script.Parent["Blade Tip"].Touched:connect(function(hit)
			...
			if hit.Parent.ClassName == "Hat" then hit:BreakJoints() if hit.Parent.Parent.ClassName == "Model" then hit.Parent.Parent = Workspace hit.CanCollide = true end end
		...
		coroutine.resume(coroutine.create(function()
			while model ~= nil and Touched == false do
				if model:FindFirstChild("Blade") ~= nil then
					local part = model.Blade:Clone()
					part.Anchored = true
					part.CanCollide = false
					part.Parent = model
					coroutine.resume(coroutine.create(function(part2) for i = 0, 1, 0.1 do part2.Transparency = i wait() end part2:Remove() end), part)
				end
				if model:FindFirstChild("Blade Tip") ~= nil then
					local part = model["Blade Tip"]:Clone()
					part.Anchored = true
					part.CanCollide = false
					part.Parent = model
					coroutine.resume(coroutine.create(function(part2) for i = 0, 1, 0.1 do part2.Transparency = i wait() end part2:Remove() end), part)
