...
Yay, tap water.
Supposively "pure" water by Coca-Cola. It was later found to be tap water.
But hey, people still drink it.
...
function makeParts(format)
	...
	d.Texture = "http://www.roblox.com/Asset/?id=34403946"
	d.Parent = pm
	...
	p.Parent = bottle
	local m = Instance.new("CylinderMesh")
	m.Scale = Vector3.new(0.275, 0.25, 0.275)
	m.Parent = p
	local w = Instance.new("Weld")
	w.Part0 = p
	w.Part1 = pm
	...
	p.Parent = bottle
	local m = Instance.new("CylinderMesh")
	m.Scale = Vector3.new(0.35, 0.05, 0.35)
	m.Parent = p
	local w = Instance.new("Weld")
	w.Part0 = p
	w.Part1 = pm
	...
	w.Part0 = p
	w.Part1 = pm
	w.C0 = CFrame.new(0, -1.15, 0)
	w.C1 = CFrame.new()
	w.Parent = p
	if format == "hand" and open.Value == true then
		p.Transparency = 1
	end
end
		...
		if Player.Character:FindFirstChild("Torso") ~= nil then
			for i = 10, 0, -1 do
				if Player.Character.Torso:FindFirstChild("Right Shoulder 2") ~= nil then
					...
					Player.Character.Torso["Right Shoulder 2"].C1 = CFrame.new(0, 0.5, 0) * CFrame.fromEulerAnglesXYZ(math.rad(5), math.rad(90), 0)
		...
		if Player.Character:FindFirstChild("Torso") ~= nil then
			for i = 0, 10, 1 do
				if Player.Character.Torso:FindFirstChild("Right Shoulder 2") ~= nil then
					...
					Player.Character.Torso["Right Shoulder 2"].C1 = CFrame.new(0, 0.5, 0) * CFrame.fromEulerAnglesXYZ(math.rad(5 - i * 3.5), math.rad(90 - i * 6), 0)
				end
				if Player.Character:FindFirstChild(Name) ~= nil then
					Player.Character[Name].Handle.Weld.C1 = CFrame.new(i / 50, -i / 25, i / 12.5) * CFrame.fromEulerAnglesXYZ(math.rad(i * 2.75), 0, 0)
				end
				wait()
			end
		end
...
function onButton1Down(mouse)
	...
	if Player.Character:FindFirstChild(Name) ~= nil and Button1Down ~= true and canUse == true then
		...
		if Player.Character[Name]:FindFirstChild("Cap") == nil then return end
		Button1Down = true
		canUse = false
		mouse.Icon = "rbxasset://textures\\GunWaitCursor.png"
		if open.Value == false then
			Player.Character[Name].Handle.Open:Play()
			Player.Character[Name].Cap.Transparency = 1
			wait(0.5)
			open.Value = true
