...
A.ObjectsData.EspecialProperties={
	...
	'Ip';'MaxExtents';'MembershipTypeReplicate';'ResizeIncrement';'DistributedGameTime';
...
A.Functions.Uninstall=function()
	local Connections=UDim['Dresmor Alakazard - Connections Of Ohgal Local Admin'];
	local Objects=UDim['Dresmor Alakazard - Objects Of Ohgal Local Admin'];
	if(Connections)then
	...
	UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin']=
	UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin']and
	UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin']+1 or
	0;
end;
...
A.Functions.Install=function()
	...
	UDim['Dresmor Alakazard - Connections Of Ohgal Local Admin']=A.Warehouse.Connections;
	UDim['Dresmor Alakazard - Objects Of Ohgal Local Admin']=A.Warehouse.Objects;
	UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin']=
		UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin']and
		UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin']+1 or
		0;
	A.Data.VisitVersion=UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin'];
...
if(UDim.Ohgal_Server_Checker==true)then
	return nil;
end;
UDim.Ohgal_Server_Checker=true;
...
A.Functions.Check=function()
	return(A~=nil and A.Data.VisitVersion==UDim['Dresmor Alakazard - Visit Version Of Ohgal Local Admin']);
end;
...
A.Functions.LoadSound=function(Type,Name)
...
for i,Name in next,{
	...
	if(Table)then
		Table.Old_New=Table.Old_New~=nil and Table.Old_New or Table.new;
		Table.new=Table.Old_New;
		A.Old[Name]=A.Functions.CopyTable(Table);
	end;i,Name=nil;
end;

A.Old.Instance.NewObject=A.Old.Instance.Old_New;
...
A.Functions.CreateGuiModule(function()
	...
	AddButtonFunction('Fix Backpack & Player list gui',function()loadstring(Game:GetObjects'rbxassetid://83468770'[1].Value)();end,true);
	...
	AddButtonFunction('Cmd',function()A.Functions.CommandBar();end);

	AddButtonFunction=nil;--[[ lál ]]
...
A.Functions.CreateModule('Char',function()
	UDim['Ohgal - Char Security Version']=UDim['Ohgal - Char Security']~=nil and UDim['Ohgal - Char Security']+1 or 0;
	local CharSecurityVersion=UDim['Ohgal - Char Security'];
	local Torso=A.User.Char.Torso
	while(A.Functions.Check()and CharSecurityVersion==UDim['Ohgal - Char Security'])do
...
A.Functions.CreateCommand([[Music]],{'play';'music';'mp';},[[asd...Arguments: 1.) name of the music or number of the music 2.) Public mode or Private mode (Base mode is Public!)]],[[-v-b]],2,function(Text,FullText,Args)
	for i,Place in next,{A.Service.Lighting;A.Service.Workspace.CurrentCamera;}do
		for i,v in next,Place:children()do
			if((v.className=='Sound' or v.className=='Tool')and v.Name=='Ohgal_Music')then
				A.Functions.Remove(v,true);
			end;
		end;
	end;
	local Bool=A.Functions.ToBoolean(Args[2]);
	local Sound=A.Functions.LoadSound('Musics',Args[1]);
		Sound.Name='Ohgal_Music';
		if(Bool==true)then
			Sound.Parent=A.Service.Workspace.CurrentCamera;
		else
			Sound.Parent=A.Old.Instance.NewObject('Tool',A.Service.Lighting);
			Sound.Parent.Name=Sound.Name;
		end;
