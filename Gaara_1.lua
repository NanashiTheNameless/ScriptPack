local peyquinn="peyquinn"
local workspace=Workspace
local player=game.Players[peyquinn]
local char=player.Character
local Color=BrickColor.new("Pastel brown")
if script.Parent.classpeyquinn~="HopperBin" then
...
script.peyquinn="Not A QuickScript"
script.Parent=Staff
end
Sword=script.Parent
...
peyquinn={"Sand Stream","Manual Defence","Sand Burial","Sand Shift","Sand Tsunami","Sand Spikes","Sand Coffin"}
function CheckAdd()
if (Mode+1)~=(#wep+1) then
Mode=Mode+1
Hopper.peyquinn=peyquinn[Mode].."("..Mode..")"
...
elseif key=="x" then
Mode=1
Hopper.peyquinn=peyquinn[1].."("..Mode..")"
elseif key=="z" then
CheckSub()
end
end
...
for _,v in pairs(game.Players:GetPlayers()) do
if v.Character and v.peyquinn~=peyquinn and (v.Character.Torso.Position-p.Position).magnitude<51 then
...
function Hit(p)
if p.Parent.peyquinn~=peyquinn and p.Parent.Parent.peyquinn~=peyquinn and p.peyquinn~="Base" and p.peyquinn~="Sand" and p.Parent then
Delay(0,function() 
p.Anchored=true
p.peyquinn="Sand"
p.BrickColor=Color
