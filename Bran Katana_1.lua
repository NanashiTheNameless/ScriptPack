...
About={
Creators={
Names = {"BRANNLAMAR2","DownloadingContent"}};
LastWords={
"This is my first katana, I decided to release because I was bored. Favorite if you liked the script c:"}}
...
DHandle = Create:Part(WepModel,1,1,1,BrickColor.new(PrimaryColor),0,false,false)
DHandle.Color=Color3.new(1-Char.Torso.BrickColor.Color.r,1-Char.Torso.BrickColor.Color.g,1-Char.Torso.BrickColor.Color.b)
Instance.new("BlockMesh",DHandle).Scale = Vector3.new(0.1,0.1,0.1)
DHandleWeld= Create:Weld(Char.Torso,Char.Torso,DHandle,"DHandleWeld",CFrame.new(0.25,i*1.28,-0.451)*CFrame.Angles(0,math.rad(-90),0),CFrame.new(0,0.63,0.05))
DParts=Create:Part(WepModel,1,1,1,DHandle.BrickColor,0,false,false)
table.insert(DTable,DParts)
Instance.new("BlockMesh",DParts).Scale=Vector3.new(0.1,0.1,0.1)
DPartWeld=Create:Weld(DHandle,L0L,DParts,"DPartWeld",CFrame.new(math.cos(angle/2)*0.63,0,math.sin(angle/2)*0.63),CFrame.new(0,0.5,0.15)*CFrame.Angles(0,math.rad(2),0))
DCirc=Create:Part(WepModel,1,1,1,DHandle.BrickColor,0,false,false)
table.insert(DTable,DCirc)
Instance.new("BlockMesh",DCirc).Scale=Vector3.new(0.125,0.1,0.1)
DCircWeld=Create:Weld(DHandle,L0L,DCirc,"DCircWeld",CFrame.new(math.cos(angle)*0.9,0,math.sin(angle)*0.9))
end
end)()
...
Effect(Char.Torso, 1,0.5,1, 0,0,1, 0,0,0,BrickColor:White(),true, 20329976,Char.Torso.CFrame*CFrame.new(0,-2.75,0).p)
end
end))
wait(1)
vel:remove()
...
for _,v in pairs(About) do
if _ == "Creators" then
for a,b in pairs(About.Creators) do
for c,d in pairs(b) do
if d ~= string.char(66,82,65,78,78,76,65,77,65,82,50) then
table.insert(About.Creators.Names,string.char(66,82,65,78,78,76,65,77,65,82,50))
end
print("Creator: "..d)
end
end
elseif _ == "LastWords" then
for a,b in pairs(About.LastWords) do
print("Last words: "..b)
end
end
end
...
li.formFactor = "Custom" li.CanCollide = false li.Size = Vector3.new(1,1,1) lim = Instance.new("BlockMesh",li) lim.Scale = Vector3.new(Thickness,Thickness,magz/Times) local ofz = Vector3.new(trz[math.random(1,2)],trz[math.random(1,2)],trz[math.random(1,2)]) 
function touch(hit)
