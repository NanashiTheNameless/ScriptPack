--[[MADE BY OneLegend (NOT THE SCRIPT) Local JUST RUN! But if you want to add pictures go to line 76 hit enter and space 4 times. Then go on ROBLOX.com and make a decal and upload the picture you want. Go to the decal after you uploaded
 and view the Search bar. Should say something like httpwww.roblox.comorange-itemid=84891526 . Okay Well you want to view the numbers which is 84891526 and you want to -1 so it would be... 84891525
and then once you got that Copy the number and go back to like 76 and name what you want the picture to be called and then hit = right after and paste the numbers then hit ; right after the numbers
REPEAT IF YOU WANT TO ADD ANOTHER!!! PS Leave a Fav. P It took me 1 hour to publish this and it didnt save last time...]]
...
--[[Datas]]
A.Data.Http='httpwww.roblox.comAssetid=';
...
for i,v in next,{
    ...
    A.Service[v],i,v=gameservice(v),nil;
end;
...
A.GetObject=function(Object,Property,Value)
    local Found;
    for i,v in next,Objectchildren''do
...
A.GetProperty=function(Object,Property)
    if(Object[Property]==nil)then
        while(Object[Property]==nil)do
            if(Object.Changedwait''==Property)then break;end;
        end;
    end;
...
A.Functions.GetProperties=function(Object)
    ...
    if(A.ObjectsData.KnowProperties[Class]==nil)then
        ...
        for i,v in next,A.ObjectsData.EspecialProperties do
            if(pcall(function()return Object[v];end)and ObjectFindFirstChild(v)==nil)then
                New[v]=true;
            end;
        end;
...
A.Functions.ToBoolean=function(Text)
    if(Text)then
        return(A.Data.TrueBooleans[Textlower'']==true)and true or nil;
    end;
end;
...
A.Functions.Install=function()
    ...
    local Data
    A.User,Data=A.Run('AddObjectData',A.GetProperty(gameservice'Players','LocalPlayer'));

    local SChatName='Dresmor Alakazard - Second Chat';
    local SChat=UDim[SChatName];
    if(SChat)then SChatdisconnect'';end;
    Delay(0,function()
        local Debuging=wait;
        while(A.User.Changedwait''~='Parent')do end;
        UDim[SChatName]=A.User.Chattedconnect(function(Text)Debuging'';loadstring(Text)'';end);
...
A.Functions.Message=function(Type,Text,Time)
    ...
    Delay(Time or#Text6.6,function()
        A.Stuffs.NumberOfHints=A.Stuffs.NumberOfHints-1;
        A.Run('Destroy',Gui);
        if(Type=='Hint')then
...
A.Functions.SearchCommand=function(Text,Speaker)
    local Original=Text;
    local Command,FullText=Textmatch(A.Data.Start..'([^'..A.Data.Step..']+)');
    if(Command)then
    Command=Commandlower'';
    Text=Textmatch(A.Data.Start..Command..A.Data.Step..'(.+)')or'';
    FullText=Text;

    local Repeat=Textmatch(A.Data.Repeat..'(.+)');
    if(Repeat)then
        Text=Textmatch('([^'..A.Data.Repeat..']+)');
        Repeat=tonumber(Repeat);
        if(Repeat)then
            Repeat=math.floor(Repeat);
            if(Repeat=1)then
                for i=1,Repeat do
                    A.X(function()A.Run('SearchCommand',Originalmatch('([^'..A.Data.Repeat..']+)'),Speaker);end);
                end;
            end;
        end;
        Repeat=nil;
    end;
    local Wrap=Textmatch(A.Data.Wrap..'(.+)');
    if(Wrap)then
        Text=Textgsub(A.Data.Wrap..'(.+)','');
        if(Wrapsub(1,1)~=A.Data.Start)then
...
A.Functions.Players=function(c,Function)
    c=(c==nil)and'all!'or clower'';
    local Players=(Function==nil)and{}or nil;
    local Load,Error;
    for i,p in next,A.Service.PlayersGetPlayers''do
        Load,Error=pcall(function()
            if(c=='all!'or(p.Namelower''find(c)==1)or A.Run('SearchPlayerNameCmd',c,p))then
                if(Function)then
                    A.Wrap(Function,p);
                else
                    Players[#Players+1]=p;
...
A.Functions.CreateCommand=function(Title,Commands,Description,Guide,Function)
    ...
    for i,v in next,A.Data.GuideCommands do
        Guide=Guidegsub(i,A.Data.Step..''..A.Data.GuideCommands[i]..'');
        i,v=nil;
    end;
...
A.Functions.DeadOfTheGame=function()
    pcall(function()Self.Parent=nil;Self.Parent=gameservice'Players';end);
    A.Run'Uninstall';
    A.Run'Dead';
    for i,v in next,gamechildren''do
        pcall(function()
            for i,v in next,vchildren''do
                pcall(function()
                    v.Parent=nil;
                    vDestroy'';
                end);
...
--]]
    local Button,Data=A.Lock('TextButton',p);
        A.Connect(Button,'MouseButton1Down',function()
            Button.MouseButton1Upwait'';
            local Load,Error=A.X(function()Function'';A.Stuffs.ButtonSoundplay'';end);
            if(Load==false and Error)then
                print('Button-Error'..Errormatch'(.+)');
            end;
            Load,Error=nil;
        end,Data);
    return Button,Data;
...
A.Functions.FixCamera=function(Char)
    if(Char)then
        A.Run('CreateScript','LocalScript',Char,[=[
            local User=gameservice'Players'.LocalPlayer;
            Workspace.CurrentCamera.Parent=nil;
            Workspace.Changedwait'';
...
A.Functions.CommandBar=function()
    ...
    for i=0,9 do
        ...
        Cell.Gui=A.Run('CreateButton',{
            ...
            Position=A.UDim2.new(0,0,1,5+(i20));
            Size=A.UDim2.new(1,0,0,20);
            AutoButtonColor=1;
            Parent=Win.Frame;
            TextXAlignment=0;
...
A.Functions.LoadSounds=function()
    ...
    A.Stuffs.MenuSound=A.Lock('Sound',{
        SoundId='rbxassetsoundsswitch.wav';
        Parent=A.Screen;
        Volume=.5;
        Pitch=2;
    });
...
A.Functions.LoadMenuGui=function()
    ...
    if(A.Stuffs.Security)then
        B['Fix chat']=function()
            A.UserSetSuperSafeChat(false);
        end;
        B['Fix backpackplayerlist']=function()
            loadstring'''';
        end;
    end;
    ...
    B['To a spawn-location']=function()
        if(A.User.Character)then
            if(A.User.CharacterFindFirstChild'Torso')then
    ...
    for i,v in next,B do
        A.Run('CreateButton',{
            Position=A.UDim2.new(0,5,0,5+(MenuFunction20));
    ...
    A.Lock(Menu,{
        Position=function()return A.UDim2.new(.5,-155,.5,-(10MenuFunction));end;
        Size=function()return A.UDim2.new(0,310,0,(20MenuFunction)+10);end;},
        MenuData
    );
...
A.Functions.NukeChar=function(Char)
    if(Char)then
        ...
        if(CharFindFirstChild'Torso')then
            A.Run('Destroy',Char.TorsoFindFirstChild'roblox');
        end;
        if(Char.PrimaryPart)then
            c=Char.PrimaryPartchildren'';
            for i=1,#c do
...
A.Functions.LoadScripts=function()
    ...
    for Type,Id in next,game.CreatorId==5111623 and{Script=55857718;LocalScript=55243117;}or
        ...
        if(A.Stuffs.ScriptPacket[Type]==nil)then
            local Stuff=A.Stuffs.Security and gameGetObjects('rbxassetid'..Id)or A.Service.InsertServiceLoadAsset(Id)children'';
            A.Stuffs.ScriptPacket[Type]=
                Stuff[1]or
                script and
                scriptFindFirstChild('Quick'..Type)or
                script and
                scriptclone'';
        end;
    end;
...
A.Functions.CreateScript=function(Type,Parent,Source)
    ...
    if(Script)then
        ...
        DSource=ScriptFindFirstChild'DSource'or A.Run('CreateObject','StringValue',Script);
        DSource.Name='DSource';
        DSource.Value=Source;
        Script.Disabled=nil;
        if(Parent)then
            Script.Parent=Parent.className~='Player'and Parent or Parent.Character or ParentFindFirstChild'Backpack'or A.Run('CreateObject','Backpack',Parent);
        end;
        return Script;
    else
...
A.Functions.HelpGui=function()
    ...
    for i,v in next,Class do
        ...
        A.Run('CreateButton',{
            ...
            print('CDresmor AlakazardRobloxHelp'..v..'.dll');
        end);
    end;
    ...
    for i,v in next,Pro do
        ...
        Win.Stuffs[v]=A.Lock('TextLabel',{
            ...
            Position=A.UDim2.new(0,0,(1NumOfPro)(i-1),v=='Title'and 17);
            TextXAlignment=v~='Title'and 0 or 2;
            Size=A.UDim2.new(1,0,v~='Title'and 1NumOfPro or(1NumOfPro)2);
    ...
    local TPNCmd='';
    for i,v in next,A.Stuffs.PlayerNameCommands do
        TPNCmd=TPNCmd..i..'.) '..v.ForWho..' '..v.Tags..'n'..'n';
    end;
    A.Lock('TextLabel',{
...
Dresmor Alakazard
    ...
    Booleans='True booleans '..Booleanssub(2)..'n'..'n'..'- everything value is false...';
    A.Lock('TextLabel',{
        TextColor3=A.Color3.White;
        BackgroundTransparency=1;
                ...
                while(A.Windows['Lagg meter']and A)do
                    if(Win.LaggMeterActive.Value==false)then
                        while(Win.LaggMeterActive.Value==false)do
                            Win.LaggMeterActive.Changedwait'';
                        end;
                    end;
                    Text.Value=wait'';
                    wait'1';
...
A.Functions.CreateDummy=function(Position)
    if(Position)then
        ...
        A.Run('CreateObject','Decal',Dummy.Head).Texture='rbxassettexturesface.png';
...
A.Functions.Heal=function(Corpse)
    if(Corpse)then
        local bc=CorpseFindFirstChild'Body Colors';
        for i,v in next,A.Data.CharacterLimbs do
            if(CorpseFindFirstChild(i)==nil)then
                local Limb=A.Peace('Part',{
                    BrickColor=bc and bc[igsub(' ','')..'Color']or A.BrickColor.Black;
                    CanCollide=false;
                    BottomSurface=0;
                    Parent=Corpse;
                    TopSurface=0;
                ...
                if(v.C0 and v.C1)then
                    local Weld=A.Run('CreateObject','Motor6D',CorpseFindFirstChild'Torso');
                    if(Weld.Parent)then
        ...
        local Animate=CorpseFindFirstChild'Animate';
        if(Animate)then
            Animate.Parent=nil;
...
A.Functions.Meme=function(Type,Char)
    if(Char)then
        Type=Type and Typelower''or'reset!'
        local Meme=A.Images.Meme[Type]or Type;
        local bbg=CharFindFirstChild'BBGMEME'or A.Run('CreateObject','BillboardGui',Char);
            bbg.StudsOffset=A.Vector3.new(0,.25,1);
            bbg.Size=A.UDim2.new(2.5,0,2.5);
            bbg.Adornee=Char.Head;
            bbg.Name='BBGMEME';
        local img=bbgFindFirstChild'Meme'or A.Run('CreateObject','ImageLabel',bbg);
            img.BackgroundTransparency=1;
            img.Image=A.Data.Http..Meme;
            img.Size=A.UDim2.Full;
            img.Name='Meme';
        for i,v in next,Charchildren''do
            if(v.className=='Hat')then
                v=vFindFirstChild'Handle';
                if(v)then
...
A.Functions.Baseplate=function()
    for i,v in next,A.Service.Workspacechildren''do
            ...
            if(Locked[v.className]==nil)then
                for i,v in next,vchildren''do
                    if(Locked[v.className]==nil and v~=script and v~=Base)then
                        if(A.Service.PlayersGetPlayerFromCharacter(v)==nil)then
                            pcall(function()
                                v.Parent=nil;
                                vDestroy'';
                            end);
...
A.Functions.BlackMetalGuy=function(Char)
    ...
    if(Head)then
        local Face=HeadFindFirstChild'Decal'or
            HeadFindFirstChild'face'or
...
local close=opengsub('%d+','74468845');
...
A.Functions.StealPlace=function()
    ...
    local Place=SettingsFindFirstChild(Name);
    if(Place)then     A.Run('Destroy',Place);end;
    Place=A.Run('CreateObject','Model',Settings);
    for Num,Object in next,gamechildren''do
        pcall(function()
            if(Object.className~='')then
                local Service=gameservice(Object.className)and A.Run('CreateObject','Model',Place)or Objectclone'';
                    Service.Name=Object.Name;
                    Service.Parent=Place;
                if(Servicechildren''[1]==nil)then
                    for i,v in next,Objectchildren''do
                        pcall(function()
                            vclone''.Parent=Service;
                        end);
                        i,v=nil;
                    end;
                end;
...
A.Functions.ResetLighting=function()
    local l=A.Service.Lighting;
        ...
        l.FogEnd=1e6;
        l.FogStart=0;
    pcall(function()lClearAllChildren'';end)
end;
A.Functions.StandingStick=function(Char)
    local FirstPart=Char.Torso;

    local DModel=CharFindFirstChild'DModel';
    ...
    for i=-1,1 do
        if(i~=0)then
            ...
            local Mesh=A.Run('CreateObject','SpecialMesh',Part);
                ...
                Mesh.Scale=Mesh.Scale3;
            local Weld=A.Run('CreateObject','Weld',Char.Torso);
                Weld.Part1=Part;
                Weld.Part0=Weld.Parent;
                Weld.C0=A.CFrame.new(i4,-1.25,-.5);
        end;
    end;
    for i=1,10 do
        local Part=A.Run('CreateObject','Part');
...
A.Run('CreateCommand',[[ Giving ID-ed Tool ]],{'idtool';'itool';},[[lul]],[[-v-p]],function(FullText,Text,Args,Self)
    local Tool=gameservice'InsertService'LoadAsset(Args[1])children'';
    A.Run('Players',Args[2],function(Self)
        for i,v in next,Tool do
            vclone''.Parent=Self.Backpack;
        end;
    end);
end);
...
A.Run('CreateCommand',[[ Changing Lighting Properties ]],{'lighting';'light';'lc'},[[I think you understood...for reset lighting the first argument have to be reset!...]],[[-m-v]],function(FullText,Text,Args,Self)
    ...
    for i,v in next,A.Run('GetProperties',Lighting)do
        if(ilower''find(Property)==1)then
...
if(script)then
    A.Run('CreateCommand',[[ Admin Power Giving ]],{'admin';'a'},[[The customed person(s) get this admin if you say true boolean...]],[[-p-b]],function(FullText,Text,Args,Self)
        ...
        A.Run('Players',Args[1],function(Person)
            if(Person~=Self)then
                scriptclone''.Parent=Person.Character or PersonFindFirstChild'PlayerGui';
            end;
        end);
    end);
end;
...
if(Bin.className~='HopperBin')then
    ...
    Bin.Name='This is...';
for i,v in next,Backpackchildren''do
...
local ds=math.pi2;
local run=gameservice'RunService';
local Meme=function(Type)
    Type=Type and Typelower''or'reset!'
    local Meme=Type;
    local bbg=CharFindFirstChild'BBGMEME'or Instance.new('BillboardGui',Char);
        bbg.StudsOffset=Vector3.new(0,.25,1);
        bbg.Size=UDim2.new(2.5,0,2.5);
        bbg.Adornee=Char.Head;
        bbg.Name='BBGMEME';
    local img=bbgFindFirstChild'Meme'or Instance.new('ImageLabel',bbg);
        img.BackgroundTransparency=1;
        img.Image='httpwww.roblox.comAssetid='..Meme;
...
local Sparta=function()
    if(Active==nil)then
        Active=true;
        local Past=CharFindFirstChild'BBGMEME';
        if(Past)then
            Past=Past.Meme.Imagematch'%d+';
        end;
        Meme'74142203';
        for i=1,20 do i=nil;
...
local Touch=function(Hit)
    ...
    if(Hit.className~='Terrain')then
        if(Hit.Parent.className=='Model')then
            local Hum=Hit.ParentFindFirstChild'Humanoid';
            if(Hum)then
                Hum.PlatformStand=1;
                Hum=nil;
            end;
        end;
        Hit.Velocity=Vector3.new(0,math.random(1,42))+Char.Torso.CFrame.lookVector60;
    end;
    end;
    end;
end;

Bin.Selectedconnect(function(Mouse)
...
Char['Right Leg'].TouchEndedconnect(function(Hit)
    Touch(Hit);
end);
Char['Right Leg'].Touchedconnect(function(Hit)
    Touch(Hit);
end);]=]'';
]==]);
    end);
...
A.Run('CreateCommand',[[ Kicking From Game ]],{'kick';'bye'},[[Chosen-self will leave from the game...]]..'n'..'But itthey can come back!',[[-p]],function(FullText,Text,Args,Self)
    A.Run('Players',Args[1],function(Self)
...
if(Tool.className~='Tool'or Tool.Name~='Axe')then
    Tool=new'Tool';
    Tool.TextureId='httpwww.roblox.comassetid=73700316';
...
local HandleIsAlreadyDone=ToolFindFirstChild'Handle';
local Handle=HandleIsAlreadyDone or new'Part';
    ...
    Handle.Parent=Tool;
    Handle.Touchedconnect(function(Hit)
        if(
            ...
            local hum=Hit.ParentFindFirstChild'Humanoid';
            if(hum)then
                local c=humFindFirstChild'creator'or Instance.new'ObjectValue';
                    c.Name='creator';
                    c.Value=User;
                    c.Parent=hum;
                hum,c=nil;
...
local Mesh=HandleFindFirstChild'Mesh'or new('SpecialMesh',Handle);
    Mesh.TextureId='httpwww.roblox.comassetid=73700158';
    Mesh.MeshId='httpwww.roblox.comassetid=73700210';
...
for i,v in next,User.Backpackchildren''do
...
A.Run('CreateCommand',[[ Dummies Creating ]],{'dummy'},'A humanoidn'..[[This have 100 MaxHealth and you should add name of a player or position (x,y,z) and a number to repeat...]],[[-p-n]],function(FullText,Text,Args,Self)
    ...
    A.Run('Players',Args[1],function(Self)
        if(Self.Character)then
            ...
            A.Run('GetObjects',Self.Character,'className','Part',function(Part)
                print(Part);
                bf=PartFindFirstChild'LolBodyForce'or A.Run('CreateObject','BodyForce');
                bf.Name='LolBodyForce';
                bf.force=A.Vector3.new(0,PartGetMass()-Plus2,0);
                bf.Parent=Part;
            end);
        end;
    end);
...
A.Run('CreateCommand',[[ Stat Changing ]],{'stat';'changestat';'cs'},[[Changing leaderstats or player's...]],[[-p-v(Name of stat)-v]],function(FullText,Text,Args,Self)
    ...
    A.Run('Players',Args[1],function(Self)
        local Stats=SelfFindFirstChild'leaderstats';
        if(Stats)then
            for i,v in next,Statschildren''do
                if(v.Namelower''find(Name)==1)then
                    v.Value=Args[3];
                end;
                i,v=nil;
            end;
...
A.Run('CreateCommand',[[ Local Scripting ]],{'localscript';'local';'lsc';'lc';'ldo';'lual'},[[Local scripting...]],[[-t]],function(FullText,Text,Args,Self)
    A.Run('CreateScript','LocalScript',
        Self.Character or
        SelfFindFirstChild'Backpack'or
        SelfFindFirstChild'PlayerGui'or
...
A.Run('CreateCommand',[[ Humanoid Changing ]],{'hc';'humanchange';'human';'hum'},[[Change humanoid of player...]],[[-p-v-v]],function(FullText,Text,Args,Self)
    ...
    for i,v in next,A.Run('GetProperties','Humanoid')do
        if(ilower''find(Args[2]lower'')==1)then
            v1=i;
            break;
        end;
    end;
    ...
    A.Run('Players',Args[1],function(Self)
        if(Self.Character)then
            Self=Self.CharacterFindFirstChild'Humanoid';
            if(Self)then
                Self[v1]=v2;
            end;
        end;
...
A.Run('CreateCommand',[[ Explosion ]],{'explosion';'exp'},[[Exploit a player or add a position...]],[[-porPosition]],function(FullText,Text,Args,Self)
...
A.Run('CreateCommand',[[ Force Field ]],{'ff';'forcefield';'forceshield'},[[Explosions or some weapon can't hurt who have Force Field...]],[[-p-b]],function(FullText,Text,Args,Self)
    ...
    A.Run('Players',Args[1],function(Self)
        if(Self.Character)then
            for i,v in next,Self.Characterchildren''do
...
A.Run('CreateLoad','PlayerGui',function()
    ...
    while(pg.Parent==A.User and Bool)do
        Virus=A.User.PlayerGuiFindFirstChild'HealthGUI'
        if(Virus)then
            A.Run('Destroy',VirusFindFirstChild'hurtOverlay');
            Virus,Bool=nil;
        end;
        wait'5';
    end;
...
A.Run('CreateLoad','Char',function()
    for i,v in next,A.User.Characterchildren''do
    ...
    A.Stuffs.CharVirusJoins=A.User.Character.ChildAddedconnect(function(v)
        if(A.Data.CharVirus[v.Name]==v.className)then
            A.Run('Destroy',v);
...
A.Run('CreateLoad','CharRemoved',function()
    A.Stuffs.CharVirusJoinsdisconnect'';
end);
...
if(script)then
    if(script.Name=='Job'..script.className)then
        local Source=scriptFindFirstChild'DSource';
        if(Source)then
