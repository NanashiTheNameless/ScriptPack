...
if Character:findFirstChild("Amaterasu", true) ~= nil then
	Character:findFirstChild("Amaterasu", true).Parent = nil
end
...
Bin = script.Parent
if Bin.Name == "Obsidian: Enigmatic" then
	Bin.Name = "Amaterasu"
end
...
attackone = function()
	...
	for i = 0, 1, 0.1 do
		...
		if Stagger.Value ~= true and Stun.Value < StunT.Value and StaggerHit.Value == true then
			break
		end
	end
	do
		...
		for i = 0, 1, 0.08 do
			...
			hitboxCF = prt11.CFrame * cf(0, 0.5, 0)
			hitbox.CFrame = hitboxCF
			MagniDamage(hitbox, 8, 7, 9, math.random(2, 5), "Normal", RootPart, 0.5, 2, (math.random(3, 5)), nil, true)
...
Obelisk = function()
	...
	attack = true
	local cando = true
	if #Obelisks > 0 and mana.Value < 15 then
		cando = false
	end
	if cando == true then
		for i = 0, 1, 0.1 do
		...
		Damagefunc(hit, 5, 10, math.random(2, 5), "Normal", RootPart, 0.5, 2, (math.random(5, 10)), nil, true)
	end
)
			hbwld.Parent = nil
			hboxpos.Parent = hitbox
			...
			do
				...
				if hitfloor ~= nil then
					...
					local omsh2 = mesh("BlockMesh", oprt2, "", "", vt(0, 0, 0), vt(10, 35, 10))
					local omsh6 = mesh("SpecialMesh", oprt6, "FileMesh", "9756362", vt(0, 0, 0), vt(2, 3, 2))
					local omsh7 = mesh("SpecialMesh", oprt7, "FileMesh", "9756362", vt(0, 0, 0), vt(4, 4, 4))
					local omsh8 = mesh("SpecialMesh", oprt8, "FileMesh", "9756362", vt(0, 0, 0), vt(2, 2, 2))
					local owld2 = weld(oprt2, oprt2, oprt1, euler(0, 0, 0) * cf(0, 0, 0))
					local owld6 = weld(oprt6, oprt6, oprt2, euler(0, 0.785, 0) * cf(0, -3.5, 0))
					local owld7 = weld(oprt7, oprt7, oprt2, euler(0, 0.785, 0) * cf(0, 3.5, 0))
					local owld8 = weld(oprt8, oprt8, oprt6, euler(0, 0.785, 0) * cf(0, -2, 0))
					eul = 0
					for i = 1, 4 do
						local oprt3 = part(3, obe, 0, 0, BrickColor.new(NewCol2), "Obelisk Part03", vt())
						local omsh3 = mesh("SpecialMesh", oprt3, "Wedge", "nil", vt(0, 0, 0), vt(10, 35, 5))
						local owld3 = weld(oprt3, oprt3, oprt2, euler(0, 0, 0) * cf(0, 0, 1.5) * euler(0, eul, 0))
						local oprt4 = part(3, obe, 0, 0, BrickColor.new(NewCol2), "Obelisk Part04", vt())
						local omsh4 = mesh("SpecialMesh", oprt4, "FileMesh", "11294911", vt(0, 0, 0), vt(2, 14.2, 2))
						local owld4 = weld(oprt4, oprt4, oprt3, euler(0, 1.57, 0) * cf(1.5, -0.05, 0))
						local oprt5 = part(3, obe, 0, 0, BrickColor.new("Black"), "Obelisk Part05", vt())
						local omsh5 = mesh("SpecialMesh", oprt5, "Wedge", "nil", vt(0, 0, 0), vt(4, 35, 3))
						local owld5 = weld(oprt5, oprt5, oprt3, euler(0.1, 0, 0) * cf(0, 1, 0.3))
						local oprt9 = part(3, obe, 0, 0, BrickColor.new("Black"), "Obelisk Part09", vt())
						local omsh9 = mesh("SpecialMesh", oprt9, "Wedge", "nil", vt(0, 0, 0), vt(4, 25, 3))
						local owld9 = weld(oprt9, oprt9, oprt3, euler(2, 0, 0) * cf(0, 2, 0.2))
						eul = eul + 1.57
					end
					oprt1.CFrame = refsate.CFrame
					so("515578896", oprt1, 1, 1)
					so("431100819", oprt1, 1, 1)
					...
					MagniDamage(oprt1, 20, 0, 0, math.random(-40, -30), "Knockdown2", RootPart, 0, 1, (math.random(5, 8)), nil, nil, true, "Defense", 0.05, 300)
...
do1 = function()
	...
	if cooldownmax <= cooldowns[1] and mana.Value >= 10 and #Obelisks > 0 then
		attack = true
		cooldowns[1] = 0
		for i = 0, 1, 0.1 do
			swait()
			...
			for i = 0, 1, 0.1 do
				...
				local mooouse = MMouse.Hit.p
				for i = 1, #Obelisks do
					Part = Obelisks[i]:findFirstChild("Obelisk Part06")
					if Part ~= nil then
						so("412831137", Part, 0.5, 1)
						so("228343271", Part, 1, 0.9)
					end
				end
				...
				for d = 1, 20 do
					swait()
					if Stagger.Value == true and Stun.Value < StunT.Value then
						...
						for i = 1, #Obelisks do
							Part = Obelisks[i]:findFirstChild("Obelisk Part06")
							if Part ~= nil then
								local MouseLook = cf((Part.Position + mooouse) / 2, mooouse)
								local hit, pos = rayCast(Part.Position, MouseLook.lookVector, 100, Character)
								local mag = (Part.Position - pos).magnitude
								ref2.CFrame = cf(pos)
								MagicCylinder3(BrickColor.new(NewCol2), CFrame.new((Part.Position + pos) / 2, pos) * angles(1.57, 0, 0), 7, mag * 5, 7, 2, 1, 2, 0.1)
								MagicBlock(BrickColor.new(NewCol2), cf(pos), 5, 5, 5, 5, 5, 5, 0.1, 1, workspace)
								if d % 3 == 0 then
									so("144699494", ref2, 1, 1)
									MagicRing(BrickColor.new(NewCol2), cf(pos) * euler(math.random(-50, 50), math.random(-50, 50), math.random(-50, 50)), 1, 1, 1, 1.5, 1.5, 0, 0.05)
									MagicCircle(BrickColor.new(NewCol2), cf(pos), 1, 1, 1, 5, 5, 5, 0.05)
									MagniDamage(ref2, 8, 4, 6, 0, "Normal", RootPart, 0.3, 2, (math.random(3, 4)), nil, nil, true, "Obsidian", 0.02, 200)
								end
							end
						end
					end
		...
		do
			...
			do
				...
				for i = 1, 15 do
					...
					if hit ~= nil then
						MagicCircle(BrickColor.new(NewCol2), cf(pos), 5, 0, 5, 1, 25, 1, 0.075)
						MagicCircle(BrickColor.new(NewCol2), cf(pos), 0, 0.2, 0, 5, 0, 5, 0.075)
						MagicCircle(BrickColor.new(NewCol2), cf(pos), 0, 0.2, 0, 15, 0, 15, 0.1)
						MagicCircle(BrickColor.new(NewCol2), cf(pos), 5, 0, 5, 3, 75, 3, 0.1)
						local ref2 = part(3, workspace, 0, 1, BrickColor.new("Black"), "Effect", vt())
						ref2.Anchored = true
						ref2.CFrame = cf(pos)
						so("157878578", ref2, 1, 1)
							...
							if ThePrt ~= nil then
								...
								if mag <= 8 then
									...
									if Prt1 ~= nil then
										...
										MagniDamage(Prt1, 15, 10, 15, math.random(-50, -40), "Knockdown2", RootPart, 0, 1, (math.random(15, 25)), nil, nil, true, "Defense", 0.05, 300)
									...
									do
										do
											...
											-- DECOMPILER ERROR at PC787: LeaveBlock: unexpected jumping out DO_STMT
						...
						MagniDamage(ref2, 8, 4, 6, math.random(-40, -30), "Knockdown2", RootPart, 0.3, 1, (math.random(4, 6)), nil, nil, true, "Obsidian", 0.1, 300)
						for i = 1, 5 do
					...
					do
						do
							poss = poss * cf(0, 0, -3)
							refb.CFrame = poss
							-- DECOMPILER ERROR at PC958: LeaveBlock: unexpected jumping out DO_STMT
...
do3 = function()
	...
	if cooldownmax <= cooldowns[3] and mana.Value >= 30 then
		...
		do
			so("181004943", Torso, 1, 0.6)
			so("402981977", Torso, 1, 0.6)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 435, 435, 435, -9, -9, -9, 0.015)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 870, 870, 870, -18, -18, -18, 0.015)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 300, 0.5, 300, -6, 0, -6, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 325, 0.4, 325, -6.25, 0, -6.25, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 350, 0.3, 350, -6.5, 0, -6.5, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 375, 0.2, 375, -6.75, 0, -6.75, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 400, 0.1, 400, -7, 0, -7, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 425, 0.4, 425, -7.25, 0, -7.25, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 450, 0.3, 450, -7.5, 0, -7.5, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 475, 0.2, 475, -7.75, 0, -7.75, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 500, 0.1, 500, -8, 0, -8, 0.02)
			for i = 1, 50 do
				swait()
			...
			dec2.Parent = nil
			passive.Value = -1
			so("87767777", Torso, 3, 1)
			so("178452221", Torso, 3, 0.6)
			so("181004943", Torso, 3, 1)
			so("138186576", Torso, 3, 1)
			MagicWave(BrickColor.new(NewCol2), cf(RootPart.Position) * cf(0, 2, 0), 2.5, 10, 2.5, 2.5, 0.02, 2.5, 0.05)
			MagicWave(BrickColor.new(NewCol2), cf(RootPart.Position) * cf(0, -2, 0), 5, 10, 5, 5, -0.02, 5, 0.05)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 8, 8, 8, 24, 24, 24, 0.05)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 75, 0, 75, 2, 175, 2, 0.005)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 80, 0, 80, 2.5, 175, 2.5, 0.01)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 400, 0, 400, 0, 200, 0, 0.075)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 500, 0, 500, 0, 150, 0, 0.07)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 600, 0, 600, 0, 100, 0, 0.065)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 700, 0, 700, 0, 50, 0, 0.06)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 7, 7, 7, 21, 21, 21, 0.045)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 6, 6, 6, 18, 18, 18, 0.04)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 5, 5, 5, 15, 15, 15, 0.035)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 4, 4, 4, 12, 12, 12, 0.03)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 3, 3, 3, 9, 9, 9, 0.025)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 2, 2, 2, 6, 6, 6, 0.02)
			MagicCircle(BrickColor.new(NewCol2), cf(Torso.Position), 1, 1, 1, 3, 3, 3, 0.015)
			MagniDamage(RootPart, 50, 9, 22, math.random(30, 50), "Knockdown2", RootPart, 0, 1, (math.random(4, 8)), nil, nil, false)
			for i = 0, 1, 0.1 do
				swait()
...
do4 = function()
	...
	if cooldownmax <= cooldowns[4] and mana.Value >= 20 and #Obelisks > 0 then
		attack = true
		cooldowns[4] = 0
		local dec = Instance.new("NumberValue", Decrease)
		dec.Name = "DecreaseMvmt"
		...
		for b = 1, #Obelisks do
			...
			MagniDamage(prt18, 30, 5, 8, math.random(-40, -20), "Knockdown2", prt18, 0, 1, (math.random(5, 8)), nil, nil, false, "Obsidian", 0.05, 400)
			ref = part(3, workspace, 0, 1, BrickColor.new("Black"), "Reference", vt())
			ref.Anchored = true
			ref.CFrame = cf(Torso.Position)
			game:GetService("Debris"):AddItem(ref, 1)
			so("153832545", ref, 2, 1)
			so("416315847", Torso, 1, 1)
			if Obelisks[b] ~= nil then
				Part = Obelisks[b]:findFirstChild("Obelisk Part01")
				if Part ~= nil and Rooted.Value == false then
					Torso.CFrame = cf(Part.Position) * cf(0, math.random(50, 100) / 100, 0) * euler(0, math.random(-50, 50), 0)
					Torso.Velocity = vt(0, 0, 0)
				end
			end
		...
		d = nil
		if Obelisks[1] ~= nil then
			d = Obelisks[1]:findFirstChild("Obelisk Part06")
		end
		if d ~= nil then
			pos = d.Position
			so("157325701", d, 1, 0.75)
			ref = part(3, workspace, 0, 1, BrickColor.new("Black"), "Reference", vt())
			ref.Anchored = true
			ref.CFrame = cf(pos) * cf(math.random(-5000, 5000) / 100, 0, math.random(-5000, 5000) / 100)
			game:GetService("Debris"):AddItem(ref, 10)
			lightnum = 0
			for i = 1, 50 do
				swait()
				MagicBlock(BrickColor.new(NewCol2), cf(pos), 1, 1, 1, 3, 3, 3, 0.2, 1)
				ref.CFrame = cf(pos) * cf(math.random(-5000, 5000) / 100, 0, math.random(-5000, 5000) / 100)
				ref2 = part(3, workspace, 0, 1, BrickColor.new("Black"), "Reference", vt())
				ref2.Anchored = true
				ref2.CFrame = cf(ref.Position)
				game:GetService("Debris"):AddItem(ref2, 1)
				so("432515400", ref2, 3, 1)
				MagicCircle(BrickColor.new(NewCol2), cf(posfloor2), 0, 10, 0, 3, 0, 3, 0.025)
				MagicCircle(BrickColor.new(NewCol2), cf(posfloor2), 5, 1, 5, 1, 70, 1, 0.05)
				hitfloor2 = rayCast(ref.Position, CFrame.new(ref.Position, ref.Position - Vector3.new(0, 1, 0)).lookVector, 50, Character)
				print(hitfloor2)
				if lightnum >= 10 then
					Lightning(pos, posfloor2 + vt(0, 0, 0), 5, 2, NewCol2, 0.1, 0.4, 0.05)
				end
				lightnum = lightnum + 1
			end
			hitfloor2 = rayCast(pos - Vector3.new(0, 10, 0), CFrame.new(pos - Vector3.new(0, 10, 0), pos - Vector3.new(0, 31, 0)).lookVector, 50, Character)
			so("87767777", Torso, 3, 1)
			so("178452221", Torso, 3, 0.6)
			MagicRing(BrickColor.new(NewCol2), cf(posfloor2) * euler(1.57, 0, 0), 80, 80, 0, 0.1, 0.1, 250, 0.02)
			MagicRing(BrickColor.new(NewCol2), cf(posfloor2) * euler(1.57, 0, 0), 250, 250, 250, -10, -10, -10, 0.045)
			MagicCircle(BrickColor.new(NewCol2), cf(posfloor2), 0, 60, 0, 25, 0, 25, 0.025)
			MagicCircle(BrickColor.new(NewCol2), cf(posfloor2), 0, 40, 0, 50, 0, 50, 0.025)
			MagicCircle(BrickColor.new(NewCol2), cf(posfloor2), 0, 20, 0, 75, 0, 75, 0.025)
			MagicRing(BrickColor.new(NewCol2), cf(pos) * euler(1.57, 0, 0), 80, 80, 5, -5, -5, -1, 0.05)
			MagicRing(BrickColor.new(NewCol2), cf(pos) * euler(0, 0, 0), 80, 80, 5, -5, -5, -1, 0.05)
			MagicRing(BrickColor.new(NewCol2), cf(pos) * euler(0, 1.57, 0), 80, 80, 5, -5, -5, -1, 0.05)
			for i = 1, 20 do
				MagicObsidian(BrickColor.new(NewCol2), cf(posfloor2) * cf(math.random(-5000, 5000) / 100, 0, math.random(-5000, 5000) / 100) * euler(math.random(-50, 50), math.random(-50, 50), math.random(-50, 50)), 3, 3, 3, -0.2, -0.2, -0.2, 0.02)
			end
			ref.CFrame = cf(pos)
			MagniDamage(ref, 50, 10, 15, math.random(-70, -50), "Knockdown2", ref, 0, 1, (math.random(10, 15)), nil, nil, true, "Movement", 0.4, 400)
			ref2 = part(3, workspace, 0, 1, BrickColor.new("Black"), "Reference", vt())
			ref2.Anchored = true
			ref2.CFrame = cf(ref.Position)
			game:GetService("Debris"):AddItem(ref2, 1)
			so("516142778", ref2, 1, 0.2)
			so("416315847", ref2, 1, 0.4)
			ref.Parent = nil
		end
		swait(10)
		dec.Parent = nil
...
Orb = function(Part)
	...
	spread = vt((math.random(-1, 0) + math.random()) * 2, (math.random(-1, 0) + math.random()) * 2, (math.random(-1, 0) + math.random()) * 2) * Part.Position - MMouse.Hit.p.magnitude / 100
	local MouseLook = cf((Part.Position + MMouse.Hit.p) / 2, MMouse.Hit.p + spread)
	table.insert(Effects, {MouseLook.lookVector, "Shoot", 100, Part.Position, 7, 12, math.random(10, 20)})
end
...
Triangle = function(a, b, c)
	...
	if (edg1 > b - a.magnitude or edg1 < 0 or edg2 <= c - b.magnitude) and edg2 >= 0 then
		a = b
	else
...
Damagefunc = function(hit, minim, maxim, knockback, Type, Property, Delay, KnockbackType, incstun, stagger, staghit, ranged, DecreaseState, DecreaseAmount, Duration)
	...
	if h ~= nil and hit.Parent.Name ~= Character.Name and hit.Parent:FindFirstChild("Torso") ~= nil then
		...
		HHumanoid.PlatformStand = false
				...
				rl.P = 3000
				rl.maxTorque = Vector3.new(0.3, 0.3, 0.3) * 50000000000000
...
while 1 do
	...
	stunanim = false
						...
						if Thing[2] == "CylinderClang" then
							...
							if Thing[2] == "showDamage" then
								...
								if Thing[2] == "Obelisk" then
									if Thing[6] <= 1000 and Thing[1].Parent ~= nil then
										if 100 < #Obelisks then
											Thing[6] = 9999
										end
										Thing[6] = Thing[6] + 1
										if 10 <= Thing[3] then
										...
										if 100 <= Thing[4] then
											Thing[4] = 0
											MagniBuff(Thing[5], 20, "Defense", -0.1, 200)
											MagniDamage(Thing[5], 20, 2, 2, 0, "Normal", Thing[5], 0, 1, (math.random(1, 5)), nil, nil, true, "Defense", 0.05, 100)
										...
										for i = 1, 5 do
											MagicBlock(BrickColor.new(NewCol2), cf(Thing[5].Position) * cf(0, math.random(-600, 600) / 100, 0) * euler(math.random(-50, 50), math.random(-50, 50), math.random(-50, 50)), 8, 8, 8, 2, 2, 2, 0.05, 2)
										end
										for i = 1, 10 do
											ref = part(3, workspace, 0, 1, BrickColor.new("Black"), "Reference", vt())
											ref.Anchored = true
									...
									do
										if Thing[2] == "Shoot" then
											...
											if hit ~= nil then
												Thing[3] = 0
												Damagefunc(hit, Thing[5], Thing[6], Thing[7], "Knockdown2", RootPart, 0, 2, (math.random(5, 10)), nil, nil, true, "Obsidian", 0.05, 500)
												...
												MagniDamage(ref, 15, Thing[5] / 1.5, Thing[6] / 1.5, 0, "Normal", ref, 0, 1, (math.random(3, 8)), nil, nil, true, "Obsidian", 0.05, 300)
											end
											if Thing[3] <= 0 then
												table.remove(Effects, e)
											end
										...
										do
											do
												...
												-- DECOMPILER ERROR at PC5829: LeaveBlock: unexpected jumping out DO_STMT

												-- DECOMPILER ERROR at PC5829: LeaveBlock: unexpected jumping out DO_STMT

												-- DECOMPILER ERROR at PC5829: LeaveBlock: unexpected jumping out IF_ELSE_STMT

												-- DECOMPILER ERROR at PC5829: LeaveBlock: unexpected jumping out IF_STMT
												...
												-- DECOMPILER ERROR at PC5829: LeaveBlock: unexpected jumping out IF_THEN_STMT
		...
		-- DECOMPILER ERROR at PC6194: LeaveBlock: unexpected jumping out DO_STMT
