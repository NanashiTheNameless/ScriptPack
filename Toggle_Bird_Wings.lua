...
--+Press V to toggle wings (Don't toggle them off while flying, unless you want to fling yourself)
...
local topcolor = invcol(BrickColor.new("Really black"))

local feacolor = BrickColor.new("Really black")
...
con = game:GetService("RunService").Stepped:connect(oc(function()
        ...
        if flying and canfly then
                ...
                if moving and canfly and keysdown["0"] and lmoving then
                ...
                if keypressed[" "] and canfly and not flying and (tick()-jumptime > .05 and (tick()-jumptime < 3 or hum.Jump)) then
...
mouse.KeyDown:connect(function(key)
    if key == "v" then
		if not hidden and cantoggle then
			cantoggle = false
			for i = 1, 10 do wait()
				for i,v in pairs(game.Players.LocalPlayer.Character.Wings:GetChildren()) do
		...
		wait(0.5)
		cantoggle = true
		elseif hidden and cantoggle then
			cantoggle = false
			for i = 1, 10 do wait()
				for i,v in pairs(game.Players.LocalPlayer.Character.Wings:GetChildren()) do
					if v:IsA("Part") or v:IsA("BasePart") then
