...
function CreateParts(Parent, Format)
	...
	if Format == 1 then
		...
		Weld.C0 = CFrame.new(0.7, 0, 0.75) * CFrame.fromEulerAnglesXYZ(math.rad(90), 0, 0)
	...
	Part.Size = Vector3.new(0.41, 0.4, 0.41)
	Part.Parent = Parts
...
for _, Player in pairs(game:GetService("Players"):GetPlayers()) do
	if Player.Character ~= nil then
		if Player.Character:FindFirstChild("Head") ~= nil then
			local Distance = (Player.Character.Head.Position - Handle.Position).magnitude
...
function onKeyDown(Key, Mouse)
	...
	if Button1Down == false and CanUse == true and CheckPlayer() == true then
		...
		elseif Key == "g" then
			CanUse = false
			playSound(10209850,Player.Character.Torso,1,1)
			SetSpeed(1, 0.75, Player.Character)
			SetAngle(1, 0, Player.Character)
			local HasHit = false
			local _, HitConnection = pcall(function() return Player.Character[ModelName].Handle.Touched:connect(function(Hit)
				...
				HasHit = true
				playSound(46153268,Player.Character.Torso,1,1)
