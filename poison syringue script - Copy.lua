...
Syringe filled with a deadly poison that slowly kills its' host.
...
function onButton1Down(mouse)
	...
	if Player.Character:FindFirstChild(Name) ~= nil then
		...
		coroutine.wrap(function()
			Instance.new("Model", Model).Name = "IsPoisoned"
			for i = Model.Humanoid.Health, Model.Humanoid.MaxHealth, 5 do
				Model.Humanoid.Health = i
				wait()
			end
			Model.Humanoid.Health = Model.Humanoid.MaxHealth
			wait(1)
			local Player2 = game:GetService("Players"):GetPlayerFromCharacter(Model)
			if Player2 ~= nil then
				if Player2:FindFirstChild("PlayerGui") ~= nil then
					local Gui = Instance.new("ScreenGui")
					Gui.Name = "Poison"
					Gui.Parent = Player2.PlayerGui
			...
			while Model.Humanoid.Health > 1 and Model:FindFirstChild("IsPoisoned") ~= nil do
				Model.Humanoid.Health = Model.Humanoid.Health - 0.25
				pcall(function() Player2.PlayerGui.Poison.Black.BackgroundTransparency = Model.Humanoid.Health / Model.Humanoid.MaxHealth end)
				Model.Humanoid.WalkSpeed = (Model.Humanoid.Health / Model.Humanoid.MaxHealth) * 16
				wait()
			end
			if Model:FindFirstChild("IsPoisoned") ~= nil then
				Model.Humanoid.Health = 0
			else
				for i = Player2.PlayerGui.Poison.Black.BackgroundTransparency, 1, 0.05 do
					pcall(function() Player2.PlayerGui.Poison.Black.BackgroundTransparency = i end)
					wait()
				end
				pcall(function() Player2.PlayerGui.Poison:Remove() end)
			end
		end)()
