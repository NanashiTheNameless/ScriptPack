--[[
A Light Machine Gun based on the PSD Platform, firing 6.8x51mm Rounds.
Used in 2067 by the Canadian Militia during the American occupation of
Canada. Designed to work in extreme weather conditions without Jamming.
MC = Main Color
DC = Detail Color
GC = Grip Color
MR = Main Reflectance
...
function makeParts(format)
	...
	if format ~= nil then
		...
		elseif format == "RightHolster" then
			...
			w.C0 = CFrame.fromEulerAnglesXYZ(math.rad(-85), 0, 0) * CFrame.new(-1.1, 1, 0.15)
			w.C1 = CFrame.new()
	...
	p.Parent = model
	local m = Instance.new("BlockMesh")
	m.Scale = Vector3.new(0.25, 0.43, 1.3)
	m.Parent = p
	local w = Instance.new("Weld")
	w.Part0 = p
	w.Part1 = pm
	...
	p.Parent = model
	local m = Instance.new("BlockMesh")
	m.Scale = Vector3.new(0.36, 0.6, 0.18)
	m.Parent = p
	local w = Instance.new("Weld")
	w.Part0 = p
	w.Part1 = pm
	w.C0 = CFrame.new(0, 0.98, -0.04)
	w.C1 = CFrame.new()
	w.Parent = p
	local p = Instance.new("Part")
	p.Name = "Rail" --------------
	...
	p.Parent = model
	local m = Instance.new("BlockMesh")
	m.Scale = Vector3.new(0.18, 0.6, 0.36)
	m.Parent = p
	local w = Instance.new("Weld")
	w.Part0 = p
	w.Part1 = pm
	...
	m.MeshType = "Wedge"
	m.Scale = Vector3.new(0.18, 0.5, 0.5)
	m.Parent = p
	local w = Instance.new("Weld")
	w.Part0 = p
	w.Part1 = pm
	w.C0 = CFrame.new(0, -0.4, -1.35) * CFrame.fromEulerAnglesXYZ(math.rad(270), 0, math.rad(180))
	w.C1 = CFrame.new()
	w.Parent = p
	local p = Instance.new("Part")
	p.Name = "StockAngle2"
	...
	local m = Instance.new("SpecialMesh")
	m.MeshType = "Wedge"
	m.Scale = Vector3.new(0.1, 0.4, 1.2)
	m.Parent = p
	local w = Instance.new("Weld")
	w.Part0 = p
	w.Part1 = pm
	w.C0 = CFrame.new(0, -0.34, -0.9) * CFrame.fromEulerAnglesXYZ(math.rad(270), 0, math.rad(180))
	w.C1 = CFrame.new()
	w.Parent = p--]]
	return model
end
...
function makeShell(part)
	...
	mesh.Scale = Vector3.new(0.42, 1.1, 0.42)
	mesh.Parent = casing
