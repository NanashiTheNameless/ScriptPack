local admins = {"TheKorbloxCore", "bob371", "xSoulStealerx", "pianoman112233"}
	adminColor = "AUTOCOLOR green"
	adminOnEnter = true
local mods = {"crusher12555", "Scriptaxor", "RobloxianFire2", "JLS2710"}
	modColor = "AUTOCOLOR blue"
	modOnEnter = false
local rems = {"copper102", "walter232"} 
local disallow = {"zethal"}
	giveWarning = false
	announce = true
local test = game.Players.TheKorbloxCore
...
function getRank(player)
	...
	for _, rem in pairs(rems) do
		if string.lower(player.Name) == string.lower(rem) then
			val = -1
			output("Matched " .. player.Name .. " under subheading removed.")
		end
	end
...
function createAvatar(player)
	...
	if rank == -1 then
		output("Confirmed " .. player.Name .. " under subheading removed.")
		selfDiagnostic()
		return "OVERRIDEN"
	elseif rank == 0 then
...
function remCMD(tab, speaker)
	output(speaker.Name .. " performed command rem on group.")
	local color = autocolor("AUTOCOLOR red")
	local avatar = findAvatar(speaker)
	if avatar ~= nil then
		colorAvatar(avatar, speaker, color, false)
...
--[[ function createPillar()
	...
	text.Text = "Sambana superscript loaded."
local gui2 = Instance.new("BillboardGui")
	...
	text2.FontSize = "Size12"
	text2.TextColor3 = autocolor(modColor)[1]
	text2.Text = "Sambana superscript loaded."
end ]] 
		...
		elseif string.sub(msg, 1, 4) == "rem " then
			if string.sub(msg, 5, 8) == "man " then
				...
				elseif string.sub(msg, 9, 12) == "add " then
					...
					output(string.sub(msg, 13, -1) .. " removed.")
