...
asset="http://www.roblox.com/asset/?id=" antisplashdamagesexhack = 50000
...
fp.Reflectance=re
fp.BottomSurface=0 
fp.Name="Geomaster"..pn --fp.FrontSurface="Hinge"
...
function RandomPlayer(spiz) local torz=spis local torses={} for i,v in pairs(Serv.kx:children()) do if v ~= Serv.kx.Geomaster and v.Character ~= nil and v.Character:findFirstChild("Torso") then ti(torses,v.Character.Torso) end end if #torses > 0 then torz=torses[rn(1,#torses)] end return torz end
...
repeat w() until Serv.Geomaster.Character and Serv.Geomaster.Character:findFirstChild("Torso")
pl=Serv.Geomaster
bp=pl.Backpack
pg=pl.PlayerGui
c=pl.Character
...
local dmgzz=dmgzz2 if game:service'Players':findFirstChild(aihu.Parent.Name) == nil then dmgzz=dmgzz*5 end ti(DBDH,aihu.Parent) addmp(dmgzz/2.5) if rn(1,a.cr[1]) == 1 then aihu:TakeDamage(dmgzz*a.cr[2]) dmgzz="Crit! " ..ts(dmgzz*a.cr[2]) else aihu:TakeDamage(dmgzz) end if not pos then return end splat2(pos.Position,dmgzz) cw(function() w(0.08) tr(DBDH,ft(DBDH,aihu.Parent)) end)
end) end 
