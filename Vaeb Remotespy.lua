...
local oldncall= gameMeta.__namecall
local tabChar = "      "
	...
	if objType == "table" then
		...
		if isDict or hasTables or forceDict then
			...
			table.sort(data, function(a, b)
				...
				if bType == "string" and aType ~= "string" then return false end
...
function tableToString(t) return parseData(t, 0, false, {}, nil, false) end
...
gameMeta.__namecall = function(...)
	local args = {...}
	local self = args[1]
	local key = args[#args]
	if realMethods[key] and not _G.ignoreNames[self.Name] and _G.scanRemotes then
		incId = incId + 1
		local nowId = incId
		...
		local allPassed = {}
		for i=2, #args - 1 do
			table.insert(allPassed, 1, args[i])
		end
