...
ModelName = "SPAS-12"
Player = script:FindFirstChild("Player") ~= nil and script.Player.Value or game:GetService("Players"):FindFirstChild("xSoulStealerx")
Selected = false
Connected = false
Button1Down = false
CanUse = true
...
function onKeyDown(Key, Mouse)
    ...
    if Button1Down == false and CanUse == true and CheckPlayer() == true then
        ...
        elseif Key == "r" then
            ...
            while script.Magazine.Value < (Loaded == true and 9 or 8) and Player.Backpack.Ammo["12-gauge shell"].Value > 0 and Button1Down == false do
                SetSpeed(2, 0.5, Player.Character)
                SetAngle(2, 0, Player.Character)
                CameraSlide(math.rad(-5), 0, 0.15)
                SetAngle(2, math.rad(100), Player.Character)
                Player.Backpack.Ammo["12-gauge shell"].Value = Player.Backpack.Ammo["12-gauge shell"].Value - 1
...
function onSelected(Mouse)
    ...
    if Player.Backpack.Ammo:FindFirstChild("12-gauge shell") == nil then
        Instance.new("IntValue", Player.Backpack.Ammo).Name = "12-gauge shell"
        Player.Backpack.Ammo["12-gauge shell"].Value = 8 * 3
    end
    coroutine.wrap(function()
        while Selected == true do
            script.Parent.Name = script.Magazine.Value.. ":" ..Player.Backpack.Ammo["12-gauge shell"].Value
            wait()
        end
