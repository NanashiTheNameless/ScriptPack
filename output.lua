...
function ScrollingListBase(list,template)
        ...
        local entry_height = EntryLabel.AbsoluteSize.y > 0 and EntryLabel.AbsoluteSize.y or EntryLabel.TextBounds.y + 2
        EntryLabel.Parent = nil
        EntryLabel.ZIndex = ListFrame.ZIndex
        local list_height = 0
        local num_entries = math.floor(list_height/entry_height)
        ...
        local function update()
                ...
                for i = 1,num_entries do
                        ...
                        if item then
                                ...
                                entry.Text = text
                                entry.Position = UDim2.new(0,0,0,(i-1)*entry_height)
...
--        wait()
        ...
        ListClass.UpdateCallback = function()
                ...
                ScrollDrag.Position = UDim2.new(0,0,0,pct*(ScrollBar.AbsoluteSize.y-ScrollDrag.AbsoluteSize.y))
        end
        ...
        ScrollDrag.MouseButton1Down:connect(function(x,y)
                ...
                drag_con = MouseDrag.MouseMoved:connect(function(x,y)
                        ...
                        local bar_abs_one = bar_abs_pos + bar_abs_size - drag_abs_size
                        y = y - mouse_offset
                        y = y < bar_abs_pos and bar_abs_pos or y > bar_abs_one and bar_abs_one or y
                        y = y - bar_abs_pos
                        ListClass.ScrollPercent(y/(bar_abs_size-drag_abs_size))
                end)
                up_con = MouseDrag.MouseButton1Up:connect(function()
                        scroll_stamp = tick()
                        MouseDrag.Parent = nil
...
local Screen = Instance.new("ScreenGui",game:FindFirstChild("CoreGui",true))
Screen.Name = "Output Gui for Oxcool1's Script Builder, Break Code: math.random(bla,bla10),Type of Code: Impossible(Neva)"
list = {
}
ListObject,ListClass,Expander = ScrollingList(list,UDim2.new(0.4,0,0,212),UDim2.new(0,0,1,-260))
...
OutputTrickFrame:TweenSizeAndPosition(UDim2.new(0.4,0,0,230), UDim2.new(0,0,1,-280), Enum.EasingDirection.Out, bigEasingStyle, 0.3, true,
function(state)
...
ListObject.Visible = false
OutputTrickFrame.Visible = true
OutputTrickFrame:TweenSizeAndPosition(UDim2.new(0.4*0.75,0,0,212*0.75),UDim2.new(0.045,0,1,-230), Enum.EasingDirection.In, Enum.EasingStyle.Quart, 0.15, true,
function(state)
if state == Enum.TweenStatus.Completed then 
