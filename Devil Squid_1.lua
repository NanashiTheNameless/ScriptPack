...
local bl = New.Part(1,1,1,'Really black')bl.Material="Neon"
New.Weld(bl,Torso)
...
for i=-p3.Size.x/2+size/2,p3.Size.x/2,size do
	New.Part(size,p3.Size.y/2,0,'Really red')
...
eye = New.Part(.5,.25,.1,'Really red')
...
function tabs()
	...
	local new=function(size,pos,col)
		local obj={}
		obj.p=New.Part(1,1,1,col or "Really black")
	...
	midp=new(v3n(1.25,.5,1.25),l(hdl.c,0,3.5+.05),'Really red')
	New"CylinderMesh"{_P=midp.p}
	midp=new(v3n(1.25,.5,1.25),l(hdl.c,0,-3.5-.05),'Really red')
	New"CylinderMesh"{_P=midp.p}
	tip=new(v3n(3,5,3),l(hdl.c,0,3.5+1.5,0,PI/2))
	
	star=new(v3n(1,1,1),l(tip.c,-1.5)*ang(md'45',PI/2,PI/2),'Really red')
	...
	for i=r,m.tau+r,m.tau/8 do
		local pp=new(v3n(.1,.1,.1),l(tip.c,math.cos(i)*r,2.5,math.sin(i)*r),'Really red')
		pp.c=cfn(pp.c.p,l(tip.c,0,2.5,0).p)
		local pp=new(_V,l(tip.c,math.cos(i)*r,-2.5,math.sin(i)*r),'Really red')
		pp.c=cfn(pp.c.p,l(tip.c,0,-2.5,0).p)
	end
	end
