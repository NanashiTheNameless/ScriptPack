--[[
Stun Grenade
Knocks down and stuns any people in the immediate vicinity.
--]]
...
function CreateParts(Parent, Format)
	...
	MasterPart.Size = Vector3.new(0.5, 0.8, 0.5)
	...
	Part.Size = Vector3.new(0.515, 0.2, 0.515)
	Part.Parent = Parts
	...
	Part.BottomSurface = 0
	Part.FormFactor = "Custom"
	Part.Size = Vector3.new(0.51, 0.4, 0.51)
	Part.Parent = Parts
...
Source.CFrame = CFrame.new(Source.Position)
...
local Smoke = Instance.new("Smoke", Source)
...
for _, Player in pairs(game:GetService("Players"):GetPlayers()) do
	if Player.Character ~= nil then
		if Player.Character:FindFirstChild("Torso") ~= nil and Player.Character:FindFirstChild("Humanoid") ~= nil then
			local Distance = (Player.Character.Torso.Position - Source.Position).magnitude
			if Distance < MaxDistance then
				local Down = true
				Player.Character.Torso.Friction = 0.5
				Player.Character.Humanoid.PlatformStand = true
				Player.Character.Humanoid.Changed:connect(function(Property)
					if Property == "PlatformStand" and Player.Character.Humanoid.PlatformStand == false and Down == true then
						Player.Character.Humanoid.PlatformStand = true
					end
				end)
				wait()
				Player.Character.Torso.Velocity = Player.Character.Torso.Velocity + ((Player.Character.Torso.Position - Source.Position).unit * 75 * ((MaxDistance - Distance) / MaxDistance))
				coroutine.wrap(function()
					wait(((MaxDistance - Distance) / MaxDistance) * 10)
					Down = false
					Player.Character.Torso.Friction = 0
...
Smoke.Enabled = false
wait(5)
Source.Parent:Remove()]], "Stun", {"Source", Player.Character[ModelName].Handle})
