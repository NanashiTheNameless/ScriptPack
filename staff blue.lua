...
char.Humanoid.Died:connect(function() if char:findFirstChild("Torso")~=nil then Nova() end for _,v in pairs(player:children()) do if v:IsA("Hint") then v:remove() end  end function hint(msg,time) end end) --I HATE leftover messages.
...
bo.Parent=b
local co=b.Touched:connect(function(hit) if hit.Parent.Name~=Name and hit.Name~="Base" and hit.Name~="Handle" and hit.Parent.Parent.Name~=Name then hit.BrickColor=BrickColor:Black() for i=1,10 do hit.Transparency=i/10 wait() end pcall(function() hit:remove() end) end end)
b.Parent=workspace
Delay(3,function() b:remove() end)
end
