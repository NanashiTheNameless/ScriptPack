...
EffectModel = nil
MeleeDmg = 30
Accuracy = {Minimum = 0, Maximum = 0, Momentum = 1}
ShootingAccuracy = 1000
reloadspeed=10
attackspeed=99999999999
maxammo=9999
Selected = false
ReloadFlash = false
Attacking = false
...
Sounds = {
        ...
        Troll = {Id = "http://www.roblox.com/asset/?id=165246404", Pitch = 1, Volume = 0.5},
}
...
Notime(function()
        while true do
                ...
                if TempAcc ~= 0 then
                        ...
                        if TempAcc ~= 0 then
                                AccTimes = AccTimes - 1
                                if AccTimes < 1 then AccTimes = AccTimesMax Notime(function() swait(0.4) AccOn = false end) TempAcc = 0 end
                        end
                end
...
sanic=Instance.new("Sound")
sanic.Parent=Torso
--sanic.SoundId="http://www.roblox.com/asset/?id=149779810"
--sanic.SoundId="http://www.roblox.com/asset/?id=35930009"
--sanic.SoundId="http://www.roblox.com/asset/?id=160369451"
sanic.SoundId="http://www.roblox.com/asset/?id=165246404"
...
function Reload(once)
        while Ammunition.Ammo < Ammunition.MaxAmmo and Ammunition.Ammoleft > 0 do
                swait()
                for i = 0.33, 1, 0.33*reloadspeed do
                ...
                for i = 0.25, 1, 0.25*reloadspeed do
                        RAW.C0 = CA(MR(43-8*i), 0, MR(-15)) * CN(0, -0.2-0.2*i, 0)
                        LAW.C0 = CA(MR(50), MR(40), MR(-15)) * CN(0.5, -0.5, 0)
                        LHW.C0 = CA(MR(-35), MR(60), MR(-45)) * CN(-0.6, 0.2, -0.24)
                        swait()
                end
                for i = 0.5, 1, 0.5*reloadspeed do
                ...
                Ammunition.Ammo = Ammunition.Ammo + 50
...
function Shoot(Start, To)
        ...
        for Pellets = 1, 30 do
                local Power = 200
                local Drop = 0.2
                local Face = CN(Start, To) * Accu * CA(MR(MRA(-10000, 10000)/ShootingAccuracy), MR(MRA(-10000, 10000)/ShootingAccuracy), MR(MRA(-10000, 10000)/ShootingAccuracy))
                ...
                Notime(function()
                        ...
                        local HitPos
                        local DistThing = 250
                        for Distance = Power, 300, Power do
                                ...
                                Pellet.CFrame = Face
                                Drop = Drop + 0.04
                                local Effect = Part(EffectModel, true, false, 0, 0, BrickColor.new(Color3.new(math.random(),math.random(),math.random())) , V3(0.2, 0.2, 0.2))
                        ...
                        if HitObj then
                                ...
                                if H then
                                        ...
                                        Dmg = MRA()*500000000
                                        H.Health = H.Health - Dmg
--                                        PlaySound(Sounds.Troll)
                                        --Show_Damage(HitObj.Position, 1, 1.5)
                                else
                                        PlaySound(Sounds.Troll)
                                        Pos=HitPos
                                        ...
                                        s.Parent=p
                                        s.SoundId="http://www.roblox.com/asset/?id=165246404"
                                        s.Pitch=1
                                        s.Volume=1
                                        s:Play()
                                        end
...
--        PlaySound(Sounds.Fire, false)
        for i = 0.33, 1, 0.33*attackspeed do
        ...
        for i = 0.2, 1, 0.2*attackspeed do
...
function onSelected(mouse)
        ...
        mouse.Button1Up:connect(function() sanic:Pause() mousedown=false end) 
