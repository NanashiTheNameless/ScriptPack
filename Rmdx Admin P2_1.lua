...
ServerLoad("con = {} pr = {} pr.AntiVirus = {} pr.Log = {} ")
...
Log("RMDX Tech Admin Gui Loaded","Bright blue")
for i=1,18 do 
Log("---","White")
end
...
Antiv[6] = {"Anti-Script Execution'Prevents Future scripts from running","AntiScript","Bool",nil,function() ServerLoad("pr.AntiVirus.AntiScript=true") dive(game:service'Players',"className","LocalScript",{{"Disabled",true}}) dive(workspace,"className","Script",{{"Disabled",true}}) end,function() ServerLoad("pr.AntiVirus.AntiScript=false") end}
...
v.MouseButton1Down:connect(function() if pr.AntiVirus[Antiv[i][2]]==true then if Antiv[i][6] then Antiv[i][6]() end v.Image=ds.DCross pr.AntiVirus[Antiv[i][2]] = false Log("AntiVirus." ..se("'",Antiv[i][1])[1].. " : Off",pr.log.msg) else if Antiv[i][5] then Antiv[i][5]() end v.Image=ds.DCheck pr.AntiVirus[Antiv[i][2]] = true Log("AntiVirus." ..se("'",Antiv[i][1])[1].. " : On",pr.log.msg) end 
...
Logt = qi({"TextLabel",nil,TextWrap=true,Font="ArialBold",FontSize="Size14",TextColor3=bc(pr.tx1).Color,Size=ud(0,135,0,30),BackgroundTransparency=0.4,BackgroundColor3=bc(pr.tx2).Color,BorderSizePixel=0})
...
Logt.Parent = nl Logt.Position = ud(0,-135,0,0) Logt.Text = Clock(pr.Log[tonumber(nl.Name)][3]).. [[ 
 ]] .. math.floor(tick()-pr.Log[tonumber(nl.Name)][3]) .. " seconds ago" Logt.Name=nl.Name 
...
ServerLoad([[ 
con[#con+1] = workspace.DescendantAdded:connect(function(v) 
if pr.AntiVirus.AntiScript and (v.className == "Script" or v.className == "LocalScript") then print"=o" v:Remove() end
if pr.AntiVirus.AntiMsg2 and (v:IsA("Message") or v:IsA("Hint")) then v:Remove() end
end)
 
con[#con+1] = game:service'Players'.DescendantAdded:connect(function(v)
...
----
if pr.AntiVirus.AntiFall and p and p.Character and p.Character.Parent and p.Character:findFirstChild("Torso") and p.Character.Torso.Position.y<-100 then p.Character.Torso.CFrame=p.Character.Torso.CFrame+v3(0,120,0) p.Character.Torso.Velocity=v3(0,0,0) Log("AntiFall feature Activated",pr.log.msg) end
----
...
if v.Name == "Break " then for i,v in pairs(con) do v:disconnect() end ServerLoad("for i,v in pairs(con) do v:disconnect() end") sg:Remove() bin:Remove()
end 
end)
