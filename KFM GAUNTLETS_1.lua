...
pcall(function() char:FindFirstChild("ralf2234"):Remove() char:FindFirstChild("Belt"):Remove() end)
m = Instance.new("Model",char) m.Name = "ralf2234"
...
b1 = T.P(1.1,.3,1.1,'Really black',0,false,false,bracs)
b1.Material = 'DiamondPlate' b1.Name = 'a'
T.W(b1,v,0,-.5,0,0,0,0)
p = T.P(.3,.4,.3,'New Yeller',0,false,false,bracs) T.W(p,b1,0,0,.55,0,0,0)
p = T.P(.3,.4,.3,'New Yeller',0,false,false,bracs) T.W(p,b1,0,0,-.55,0,0,0)
p = T.P(.3,.4,.3,'New Yeller',0,false,false,bracs) T.W(p,b1,.55,0,0,0,math.pi/2,0)
p = T.P(.3,.4,.3,'New Yeller',0,false,false,bracs) T.W(p,b1,-.55,0,0,0,math.pi/2,0)
for i,v in pairs(bracs:children()) do if v.Name ~= 'a' then v.Material = 'Ice' end end
end
 
--------MAKING--------------------
h1 = T.P(1.5,1.5,1.5,'Light orange',0,false,false)
T.FM(h1,'sphere',1,1,1)
T.W(h1,char.Head,0,0,0,0,0,0)
 
e1 = T.P(.5,.5,.5,'Institutional white',0,false,false) T.FM(e1,'sphere',1,1,1)
e2 = T.P(.5,.5,.5,'Institutional white',0,false,false) T.FM(e2,'sphere',1,1,1)
e1w=T.W(e1,h1,.35,0,-.55,0,0,0) T.W(e2,h1,-.35,0,-.55,0,0,0)
 
dec = Instance.new("Decal")
dec.Face = 'Front'
dec.Texture = "http://www.roblox.com/asset/?id=133540669"

b2 = T.P(2.1,.5,1.1,'New Yeller',0,false,false)
...
ef = T.P(1,1,1,'Black',0,false,false)
...
if v.Name ~= 'a' then table.insert(fires,T.F(v,.5,.5,'New Yeller','Black')) end
end
...
stick = T.P(.3,.3,2.5,'New Yeller',.5,false,false,sticks)
stick.Touched:connect(function(hit) damage(hit.Parent,10000000000,true,.05) end)
...
function bowl(mouse)
colorslide(e1,'Color','cur','Institutional white',.05)
dec.Parent = e1
light = T.P(1,2,1,'Really red',.8,false,false)
light.Touched:connect(function(hit) damage(hit.Parent,9999999999999,false,1) end)
...
light:Remove()
colorslide(e1,'Color','cur','Institutional white',.05) e1w.C1 = cfn(.35,0,-.55)
dec.Parent = nil
end
