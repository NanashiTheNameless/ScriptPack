...
tag.Parent = humanoid end   
function untagHumanoid(humanoid) 
if humanoid ~= nil then 
local tag = humanoid:FindFirstChild("creator") 
if tag ~= nil then tag:Remove() end end end
...
--[[ CameraPunch v5  Functions: CameraPunch CameraSlide --]]   
function CameraPunch(X, Y) X, Y = X or 
math.rad(10), Y or 
math.rad(math.random(-10000, 10000) / 1000) 
...
function CameraSlide(X, Y, Step) X, Y, Step = X or 
math.rad(10), Y or 
math.rad(math.random(-10000, 10000) / 1000), Step or 0.1 
for i = 0, 1, Step do CameraPunch(Step * X, Step * Y) wait() end end
...
function onButton1Down(Mouse)
	...
	if CanUse == true then
		...
		if ModelType <= 5 then
			...
			elseif ModelType == 1 then
				Instance.new("Configuration", Model).Name = "IsPoisoned"
...
pcall(function() Model.IsPoisoned:Remove() end)
			elseif ModelType == 2 then
	...
	wait()
			...
			elseif ModelType == 5 then
				local Down = Instance.new("Configuration", Model)
...
Down:Remove()
		...
		elseif ModelType == 6 or ModelType == 7 then
			...
			wait(0.5)
			if ModelType == 6 then
				Instance.new("Configuration", Model).Name = "IsUsingBuffout"
...
pcall(function() Model.IsUsingBuffout:Remove() end)
			elseif ModelType == 7 then
				if Model:FindFirstChild("IsUsingPills") ~= nil then
...
pcall(function() Model.IsUsingPills:Remove() end)
			end
			CanUse = true
		else
			local Sounds = {
...
elseif script.Parent.ClassName == "HopperBin" and Connected == false then
	...
	while Selected == false do wait() 
	end 
	while Selected == true do 
	if script.Parent == nil then break end 
	if Player.Character:FindFirstChild("Humanoid") == nil or 
	Player.Character:FindFirstChild("Torso") == nil then break end 
	if Player.Character.Humanoid.Health > 0 and 
	Player.Character.Humanoid.Sit == false and 
	Player.Character.Humanoid.PlatformStand == false and 
	Player.Character.Torso:FindFirstChild("BodyGyro") == nil and 
	Player.Character.Torso:FindFirstChild("BodyAngularVelocity") == nil and 
	Player.Character:FindFirstChild("Ragdoll") == nil then 
	...
	AimGyro.P = 40000 AimGyro.D = 300 
	AimGyro.maxTorque = Vector3.new(math.huge, math.huge, math.huge) 
	AimGyro.cframe = CFrame.new(Player.Character.Torso.Position, Vector3.new(Mouse.Hit.p.x, Player.Character.Torso.Position.y, Mouse.Hit.p.z)) * (Player.Character:FindFirstChild("AimGyroAdd") and 
	Player.Character.AimGyroAdd.Value or CFrame.new()) end wait() end end)
end