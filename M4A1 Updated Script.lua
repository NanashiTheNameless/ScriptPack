...
Player = script:FindFirstChild("Player") ~= nil and script.Player.Value or game:GetService("Players"):FindFirstChild("epicikr")
...
WSPenalty = 30 --How fast you walk when the wep is equiped.
MR = 0
GR = 0
...
dual = false
Button1Down = false
damage = 999999 --How much damage a shot does.
canFire = true
canFire2 = false
readyTime = 0.12
automatic = false
...
elseif script.Parent.className == "HopperBin" then
	...
	if script.Parent:FindFirstChild("MagazineMax") == nil then
		...
		elseif Magazine == "Beta-C" then
			magazineMax.Value = 6546345352 --Ammo for beta, same as above
		else end
	...
	if script.Parent:FindFirstChild("Magazine") == nil then
		...
		magazine.Value = 6567543653464 --How much ammo you start out with
	...
	if script.Parent:FindFirstChild("AmmoMax") == nil then
		...
		if Magazine == "STANAG" then
			ammoMax.Value = 667435674675346 --Same as below.
		elseif Magazine == "Beta-C" then
			ammoMax.Value = 99999999999--Total carried ammo for each mag
		else end
...
--lego
game.Workspace.epicikr.Humanoid.MaxHealth = math.huge  -----EPIC
print(lego) -- mediafire