--Version 7 1.02 .
adminlist = {"jjphariss","haxhax3","frankenstein212","anthonywnsyou","frankanstein212","","",""}--Frank is always admin since he made these commands.
--if your gunna copy give a thanks please.
bannedlist = { "Kinglime",""}--your banned frome my kingdome!.
texture = ""--Well we all Know this is where the t-Shirt goes lol.

--[[
the only real problem i got with peaple taking copies is they dont say hey frank thanks dude!
--]]
...
return char 
elseif cnum == 0 then
text("No such item found.",1,"Message",speaker)
return 0
elseif cnum > 1 then
...
if string.sub(string.lower(msg),1,5) == "kill/" then--command is what makes it work the most right?
local player = findplayer(string.sub(msg,6),speaker)--finds the speaker
if player ~= 0 then--this is a dodattle lol jk
for i = 1,#player do--hey bob i gtg take over for a bit
if player[i].Character ~= nil then--ok
local human = player[i].Character:FindFirstChild("Humanoid")--find there humanoid mahahahaha
if human ~= nil then--it didnt get away take it to the dungeon mahahahahaha
human.Health = 0--This part is what kills
end end end end end--This is hard...
...
if string.sub(string.lower(msg),1,2) == "c/" then--make your own script no way!!
...
if string.sub(msg,1,6) == "noarmor/" then
...
if string.sub(msg,1,4) == "armor/" then
...
local c = game.Workspace:GetChildren()
for i2 =1,#c do
local it = c[i2]:FindFirstChild("elplayerioloopkillioPlayerio")
if it ~= nil then
...
ov.Name = "elplayerioloopkillioPlayerio"
ov.Parent = script
player = ov.Value
function oa(object)
...
if string.sub(msg,1,8) == "zomby/" then
local player = findplayer(string.sub(msg,9),speaker)
if player ~= 0 then
for i = 1,#player do
if player[i].Character ~= nil then
...
--Like my friend says you must make the commands yourself or quit roblox xD.
...
if c[i].className == "Part" then
if c[i].Name == "frank's Admin Command Script V8" then
c[i]:remove()
end end
...
if string.sub(msg,1,5) == "talk/" then
local a32 = Player(speaker.Name)
if a32 == true then
local player = findplayer(string.sub(msg,6),speaker)
if player ~= 0 then
for i = 1,#player do
...
--gogle makes the best but worst games to play.
...
eloname = "frank"
eloname = eloname .. "in22"
script.Name = eloname .. "'s Admin Commands V8"
youcaughtme = 0
for i =1,#adminlist do
...
text("P,232 Loaded!.., You're an admin.",5,"Message",ack)
return
end
...
local tfv = ack.Chatted:connect(function(msg) foc(msg,ack) end)
table.insert(flist,tfv)
text("P,232 Loaded!.., Your an admin.",5,"Message",ack)
return
else
return
end 
