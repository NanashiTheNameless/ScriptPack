...
Syringe filled with a high-concentration of adrenaline.
Too much can cause heart palpitations.
...
function onButton1Down(mouse)
	...
	if Player.Character:FindFirstChild(Name) ~= nil then
		...
		if Model.Humanoid.MaxHealth > (100 * 10) * math.random(2, 5) * 10 then
			Model.Humanoid.Health = 0
			return
		end
		CanUse = false
		Model.Humanoid.MaxHealth = Model.Humanoid.MaxHealth * 10
		coroutine.wrap(function() wait(0.25) Model.Humanoid.Health = Model.Humanoid.Health * 10 end)()
		coroutine.wrap(function()
			local Max = math.random(750, 1000)
			local WalkSpeed = Model.Humanoid.WalkSpeed * 2
			for i = Max, 0, -1 do
				Model.Humanoid.WalkSpeed = WalkSpeed * (i / Max) + 16
				wait()
			end
			wait(1)
			Model.Humanoid.WalkSpeed = WalkSpeed / 2
			Model.Humanoid.MaxHealth = Model.Humanoid.MaxHealth / 10
			Model.Humanoid.Health = Model.Humanoid.Health / 10
		end)()
