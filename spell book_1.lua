...
local name = "Book"
if plr.Backpack:FindFirstChild(name) then plr.Backpack[name]:Destroy() end
...
Delay(1/60,function() s:Play() end)
...
local lh = 0
Hammer.Touched:connect(function(hit) if tick()-lh < 0.05 then return end lh = tick() local hum = hit.Parent:FindFirstChild("Humanoid") if not hum then hum = hit.Parent.Parent:FindFirstChild("Humanoid") end if not hum then return end dealDamage(hum,math.random(12,18)) end)
...
RS.C1 = RS.C1 * CFrame.new(-.1,0,0)
table.insert(Plates,makeRPlate("Brown"))
wait(.05)
RS.C1 = RS.C1 * CFrame.new(.1,0,0)
temp2 = RS.C1
local temp2 = RS.C1
