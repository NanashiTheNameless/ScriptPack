--[[{{{==~~||Sword o' souls||~~==}})]]-------------------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------ball----- If you see this, get a life hacker nerd. -----------------------------------------------------------------------------------------
name = "yfc"

hint = Instance.new("Hint") 
hint.Parent = workspace 
hint.Text = "The sword that " ..name.. " is using was made by xSoulStealerx!"
...
splashdist = 14
plat = 1
healcols = {"Mid grey", "Dark stoen grey", "Really black", "Black"}
...
prop(blade1, sword, false, 0, 0.2, 0.85, 2, 0.1, "Dark stone grey", false, "Custom")
...
prop(blade2, sword, false, 0, 0.2, 1.2, 1.2, 0.1, "Dark stone grey", false, "Custom")
b2m = Instance.new("BlockMesh",blade2)
bw2 = Instance.new("Weld")
weld(bw2, main, blade2, 0, 0, math.pi/4, 0, 3, 0)

blade3 = Instance.new("Part")
prop(blade3, sword, false, 0, 0.2, 0.1, 4.5, 0.6, "Dark stone grey", false, "Custom")
...
prop(blade4, sword, false, 0, 0.2, 0.1, 4.5, 0.6, "Dark stone grey", false, "Custom")
...
prop(blade5, sword, false, 0, 0.2, 0.6, 0.6, 0.1, "Dark stone grey", false, "Custom")
b5m = Instance.new("BlockMesh",blade5)
bw5 = Instance.new("Weld")
weld(bw5, main, blade5, 0, 0, math.pi/4, 0, 6.6, 0)

part7 = Instance.new("Part")
prop(part7, sword, false, 0.2, 0.2, 0.7, 1.6, 0.22, "Really black", false, "Custom")
...
prop(tup1, sword, false, 0, 0.4, 1.2, 0.7, 0.4, "Mid grey", false, "Custom")
tw1 = Instance.new("Weld")
weld(tw1, torso, tup1, -0.3+(math.pi/2), -0.3, 0.2+(math.pi/2), 1.1, 1.2, 0)
holdpart = Instance.new("Part")
prop(holdpart, char, false, 1, 0, 0.5, 0.5, 0.5, "Black", false, "Custom")
...
prop(rb, char, false, 1, 0, 1, 1, 1, "Black", false, "Symmetric")
rh = Instance.new("Weld")
weld(rh, rb, torso, 0, 0, 0, 1.5, 0.5, 0)

lb = Instance.new("Part")
prop(lb, char, false, 1, 0, 1, 1, 1, "Black", false, "Symmetric")
...
function blading(part, color)
	local p = Instance.new("Part",me.Character)
	prop(p,me.Character,false,0.4,0,3.8,1.4,0.2,color,true,"Custom")
	p.CFrame = part.CFrame
	coroutine.resume(coroutine.create(function()
		for i=p.Transparency, 1, 0.08 do
			wait()
			p.Transparency = i
		end
		p:remove()
...
--		block(blade5,2,{"Really black", "Mid grey", "Black", "Tr. Black"})
...
function atta()
	...
	attack = true
	for i=1,6 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(-0.54,0.28,0) * CFrame.new(0,0,0)
		weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(-0.04,0,0) * CFrame.new(0,0,0)
		neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(-0.14,0,0)
		ball(torso,"Bright red",15)
		wait()
	end
	attack = false
	wait(0.08)
...
function gra()
	...
	for i=0.4, 5.3,0.07 do
		wait(0.04)
		block(blade5,i,{"Really black", "Mid grey", "Black", "Tr. Black"})
	end
	end))
	charge:play()
	coroutine.resume(coroutine.create(function()
	...
	coroutine.resume(coroutine.create(function()
		for i=1, 12 do
			wait()
			ball(blade5,"Really black",splashdist*2)
		end
	end))
...
function grab()
	...
	for i=1,24 do
		rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(-0.2,0,0)
		lw.C0 = lw.C0 * CFrame.fromEulerAnglesXYZ(-0.2,0,0)
		neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(0.2,0,0) * CFrame.new(0,0,0)
		wait()
	end
	avgdmg = 50
	local bv = Instance.new("BodyPosition",torso)
	...
	coroutine.resume(coroutine.create(function()
		for i=1, 34 do
			wait()
			rw.C0 = rw.C0 * CFrame.fromEulerAnglesXYZ(0.25,-0.25,-0.14)
			lw.C0 = lw.C0 * CFrame.fromEulerAnglesXYZ(0.16,-0.25,0.15)
			neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(0.02,0,0) * CFrame.new(0,0,0)
		end
	end))
	coroutine.resume(coroutine.create(function()
	...
	coroutine.resume(coroutine.create(function()
		...
		for i=1, 33 do
			...
			lw.C0 = lw.C0 * CFrame.fromEulerAnglesXYZ(0.08,0,0)
			weaponweld.C0 = weaponweld.C0 * CFrame.fromEulerAnglesXYZ(0,-(math.pi/11),0)
			neck.C1 = neck.C1 * CFrame.fromEulerAnglesXYZ(-0.13,0,0) * CFrame.new(0,0,0)
		end
	end))
	criticalchance = 150
	plat = 1
	...
	coroutine.resume(coroutine.create(function()
		for i=1, 12 do
			wait()
			ball(blade5,"Really black",splashdist*10)
		end
	end))
	local parts = getcharparts(workspace)
	for _,v in pairs(parts) do
		if (v.Position - blade5.Position).magnitude < 38 then
			kill(v,true)
		end
	end
	wait(0.3)
...
function grabh()
	...
	for i=0.4, 5.3,0.07 do
		wait(0.04)
		block(blade5,i,{"Really blue", "Mid grey", "Bright blue", "Tr. Blue"})
	end
	end))
	charge:play()
	coroutine.resume(coroutine.create(function()
...
function attsa()
	...
	local bv = Instance.new("BodyVelocity",blade5)
	bv.maxForce = Vector3.new(math.huge,math.huge,math.huge)
	bv.velocity = blade5.CFrame.lookVector * 22
	local bav = Instance.new("BodyAngularVelocity",blade5)
	bav.maxTorque = Vector3.new(math.huge,math.huge,math.huge)
	bav.angularvelocity = Vector3.new(0,36,0)
	bav.P = 15000
	for i=1, 7 do
	...
	coroutine.resume(coroutine.create(function()
		for i=1, 12 do
			wait()
			ball(blade5,"Really red",splashdist*2)
		end
	end))
	local parts = getcharparts(workspace)
	for _,v in pairs(parts) do
